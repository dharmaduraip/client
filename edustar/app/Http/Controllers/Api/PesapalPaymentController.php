...
class PesapalPaymentController extends Controller
{
    ...
    {  
        ...
        $pesapal_mode = getenv('PESAPAL_MODE');
         try{
            ...
            {   
                $settings->OrderTrackingId = $result3->order_tracking_id;
                $settings->save();
                return response([
                    'redirect_url' => $result3->redirect_url
                ]);
            }
            else
            {
    ...
    {
        if($request->OrderTrackingId == "check" || $request->OrderTrackingId == "buynow")
        {   
            $settings = Setting::first();
            $user = Auth::User();
            $currency = Currency::where('default', '=', '1')->first();
            ...
            if($pesapal_mode == "live")
            {
                $url = "https://pay.pesapal.com/v3/api/Transactions/GetTransactionStatus?orderTrackingId=".$settings->OrderTrackingId;
            }
            else if($pesapal_mode == "sandbox")
            {
                $url = "https://cybqa.pesapal.com/pesapalv3/api/Transactions/GetTransactionStatus?orderTrackingId=".$settings->OrderTrackingId;
            }
            ...
            {    
                if($request->OrderTrackingId == "buynow")
                {
                    $payment_method = "pesapal";
                    $txn_id = $settings->OrderTrackingId;
                    $pesapal_token = $gsettings->pesapal_token;
                    $merchant_reference = $response_data->merchant_reference;
                    $payment_status = "1";
                    $auth_user_id = Auth::User()->id;
                    $course_id = $request->course_id;
                    return $this->buynoworder($txn_id, $payment_method, $sale_id, $file, $payment_status, $auth_user_id,$pesapal_token,$merchant_reference,$course_id,$currency_code);
                }
                if($request->OrderTrackingId == "check")
                {
                    ...
                    foreach($carts as $cart)
                    {
                        ...
                        $string =  currency($pay_amount, $from = $currency->code, $to = $currency_code, $format = true);
                        ...
                        $created_order = Order::create([
                            ...
                            'merchant_reference' => $response_data->merchant_reference,
                            'transaction_id' => $settings->OrderTrackingId,
                            'pesapal_token' => $gsettings->pesapal_token,
                            'payment_method' => "pesapal",
    ...
    public function buynoworder($txn_id, $payment_method, $sale_id = NULL, $file = NULL, $payment_status = NULL, $auth_user_id = NULL,$pesapal_token = NULL,$merchant_reference = NULL,$course_id,$currency_code)
    {
