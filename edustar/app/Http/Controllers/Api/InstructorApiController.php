...
class InstructorApiController extends Controller
{
        ...
        if (CourseChapter::where('id', $id)->exists()) {
            ...
            if($file = $request->file('file'))
            {
                if($data->file != "")
                {
                    ...
                    if($chapter_file)
                    {
                        unlink(public_path().'/files/material/'.$data->file);
                    }
                }
                $name = time().$file->getClientOriginalName();
                $file->move(public_path().'/files/material', $name);
                $input['file'] = $name;
            }
    ...
    public function createpreviouspaper(Request $request) {
        ...
        if($file = $request->file('file'))
        {
            ...
            $filename = time().$file->getClientOriginalName();
            $file->move(public_path().'/files/papers',$filename);
            $input['file'] = $filename;
        }
    ...
    public function updatepreviouspaper(Request $request, $id)
    {
        ...
        if($file = $request->file('file'))
        {
            if($data->file != "")
            {
                ...
                if($chapter_file)
                {
                    unlink(public_path().'/files/papers/'.$data->file);
                }
            }
            $name = time().$file->getClientOriginalName();
            $file->move(public_path().'/files/papers', $name);
            $input['file'] = $name;
        }
    ...
    public function createinstitute(Request $request)
    {
        ...
        if ($file = $request->file('image')) {
            ...
            if ($file = $request->file('image')) {
                $name = time() . $file->getClientOriginalName();
                $file->move(public_path().'/files/institute', $name);
