...
class ViewmoreController extends Controller
{
    ...
    public function bestselling(){
        ...
        $bestselling = Order::whereNotNUll('course_id')->with('courses','courses.user')->groupBy('course_id')->get();
    ...
    public function toprecent(Request $request){
        
        $ipaddress = $request->getClientIp();
        $geoip = geoip()->getLocation($ipaddress);
        $usercountry = strtoupper($geoip->country);
        $cors = Course::where('status', '1')->with('user')->latest()->where('created_at', '>', now()->subDays(15)->endOfDay())->get()->map(function($c) use($usercountry) {
        ...
        return view('front.viewmore.recent',compact('cors'));
    }
}
