...
class ComingSoonController extends Controller
{
    ...
    // {
    //     $this->middleware('permission:coming-soon.manage', ['only' => ['show','update','comingsoonpage']]);
    // }
    ...
    public function update(Request $request)
    {
        $this->validate($request,[
            'bg_image' => 'mimes:jpg,jpeg,png,bmp,tiff'
        ]);
        ...
        if(isset($items))
        {
            if ($file = $request->file('bg_image'))
            {
                ...
                if($image_file)
                {
                    unlink(public_path().'/images/comingsoon/'.$items->bg_image);
                }
                $name = time().$file->getClientOriginalName();
                $file->move(public_path().'/images/comingsoon', $name);
                $input['bg_image'] = $name;
            }
        ...
        // return back()->with('message',trans('flash.Updated Successfully'));
    }
