...
class SettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'APP_URL' => 'required',
            'contact_image' => 'mimes:jpeg,png,jpg,gif,svg|max:2048',
        ...
        if ($domain == 'localhost' || strstr($domain, '.test') || strstr($domain, 'dev.abserve.tech') || strstr($domain, '192.168.') || strstr($domain, 'mediacity.co.in') || strstr($domain, 'castleindia.in')) {
            return $this->extraupdate($request);
    ...
    public function extraupdate($request)
    {
        ...
        if ($file = $request->file('contact_image')) {
            ...
            $file->move(public_path().'/images/contact', $name);
    ...
    public function loginupdate(Request $request)
    {
        ...
        try {
          ...
          {        
            ...
            return redirect()->route('settings.login')->with('success', trans('flash.UpdatedSuccessfully'));
