...
class GuestController extends Controller
{
    ...
    public function addtocart(Request $request, $id)
    {
    	...
    	return back()->with('success',trans('Cart Added'));
    }
    ...
    public function removefromcart(Request $request, $id)
    {
        ...
        return back()->with('delete',trans('Cart Removed'));
    }
    public function guestlogin(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            if (session()->has('cart.add_to_cart'))
            {
                ...
                {
                    $course = Course::findorFail($c);
                    $exist_cart = Cart::Where('user_id',Auth::user()->id)->where('course_id',$c)->first();
                    if(empty($exist_cart))
                    {
                        $exist_order = Order::Where('user_id',Auth::user()->id)->where('course_id',$c)->orderBy('created_at','desc')->first();
                        ...
                        {
                            if($exist_order->enroll_expire < now())
                            {
                                $cart = new Cart;
                                $cart->user_id = Auth::user()->id;
                                $cart->course_id = $course->id;
        ...
        {
            return redirect()->back()->with('message','Email or Password that you have entered is incorrect.');
        }
    }
}
