...
class InstituteController extends Controller
{
    ...
    {
        //return $id;
        $course = Course::where('institude_id',$id)->first();
        if(!empty($course))
        {
            return back()->with('warning', 'you cannot delete this institute. Because, This institute is already used in some course!');
        }
    ...
    public function save(Request $request)
    {
        ...
        if ($file = $request->file('image')) {
            ...
            $validator = Validator::make(
                ...
                [
                    'file' => 'required',
                    'extension' => 'required|mimes:jpg,png,jpeg',
                ]
            );
    ...
    public function status(Request $request)
    {
        if($request->status == "0")
        {    
            $course = Course::where('institude_id',$request->id)->first();
            if(!empty($course))
            {
