...
class FollowersController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('permission:followers.manage', ['only' => ['index']]);
    ...
    public function unfollow(Request $request)
    {
        ...
        if(Auth::check())
        {
            $follower = Followers::where('user_id', $auth->id)->where('follower_id', $request->instructor_id)->delete();
            Session::flash('success',trans('flash.UpdatedSuccessfully'));
            return back();
        }
        else
    ...
    public function index(Request $request)
    {
        ...
        $followings = Followers::with('user','follow')->where('user_id', auth::user()->id)->where('follower_id','!=', auth::user()->id)->get();
