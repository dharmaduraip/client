...
class InstructorController extends Controller
{
    ...
    //    return auth()->user()->getRoleNames()->toArray();
        if(Auth::User()->role == "instructor")
        {
            ...
            $users =    Order::where('instructor_id',Auth::user()->id)->select(DB::raw("COUNT(*) as count"))
                ->whereYear('created_at',date('Y'))
                ->groupBy(DB::raw("Month(created_at)"))
                ->pluck('count');
            
            $months =   Order::where('instructor_id',Auth::user()->id)->select(DB::raw("Month(created_at) as month"))
                    ->whereYear('created_at',date('Y'))
                    ->groupBy(DB::raw("Month(created_at)"))
                    ->pluck('month');
