       ...
       <form action="{{route('start.quiz.store',$topic->id) }}" method="POST" id="question-form">
            {{ csrf_field() }}
                    ...
                    <div class="quiz-tabs">
                      ...
                      <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade show" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                          <img src="{{ asset('images/quiz/'.$que[0]['question_img']) }}" class=" img-fluid" alt="">
                        </div>
                        @endif
                      </div>
              ...
              <div class="jumbotron" id="quiz{{ $key+1 }}" >
                  ...
                  @if($equestion['c'] != NULL && $equestion['c'] != '')
                    <label class="radio">
                       <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="C">{{ $equestion['c'] }}
                    </label>
                  @endif
                  @if($equestion['d'] != NULL && $equestion['d'] != '')
                    <label class="radio">
                       <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="D">{{ $equestion['d'] }}
                    </label>
                  @endif
                  @if($equestion['e'] != NULL && $equestion['e'] != '')
                    <label class="radio">
                       <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="E">{{ $equestion['e'] }}
                    </label>
                  @endif
                   <br>
                </div>
              ...
              <div class="col-lg-4">
                 <div class="quiz-tabs">
                      ...
                      <ul class="nav nav-tabs" id="myTab" role="tablist">
                        ...
                        <li class="nav-item">
                          <a class="nav-link active" id="home-tab{{ $key }}" data-toggle="tab" href="#home{{ $key }}" role="tab" aria-controls="home" aria-selected="true">{{ __('Video')}}</a>
                        </li>
                        @endif

                        @if(isset($equestion['question_img']))
                        <li class="nav-item">
                          <a class="nav-link" id="profile-tab{{ $key }}" data-toggle="tab" href="#profile{{ $key }}" role="tab" aria-controls="profile" aria-selected="false">{{ __('Image')}}</a>
                        </li>
                      ...
                      <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade" id="profile{{ $key }}" role="tabpanel" aria-labelledby="profile-tab{{ $key }}">
                          <img src="{{ asset('images/quiz/'.$equestion['question_img']) }}" class=" img-fluid" alt="">
                        </div>
    ...
    if (document.getElementById) { 
        ...
        if (mins < 0) { 
            ...
            if(count == 0){
              ...
              window.location.href='{{ route('course.content',['id' => $topic->courses->id, 'slug' => $topic->courses->slug ]) }}';
              // return false;
            }
