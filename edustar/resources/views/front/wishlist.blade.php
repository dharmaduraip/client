            ...
            <div class="col-lg-3 col-md-6">
                <div class="view-block">
                    ...
                    <div class="view-user-img">
                        ...
                        <a href="{{ route('all/profile',$wish->user->id) }}" title=""><img src="{{ asset('images/user_img/'.$wish->user['user_img']) }}" class="img-fluid user-img-one" alt=""></a>
                        @else
                        <a href="{{ route('all/profile',$wish->user->id) }}" title=""><img src="{{ asset('images/default/user.png') }}" class="img-fluid user-img-one" alt=""></a>
                        @endif
                    </div>
                    ...
                    @endif
                    <div class="view-dtl">
                        <div class="view-heading"><a href="{{ route('user.course.show',['id' => $wish->courses->id, 'slug' => $wish->courses->slug ]) }}">{{ str_limit($wish->courses->title, $limit = 35, $end = '...') }}</a></div>
                        <div class="user-name">
                    ...
                    <div class="protip-wishlist">
                        <ul>
                            <li class="protip-wish-btn">
                                @if($wish->courses->type == 1 && Auth::User()->role != "admin" && $wish->courses->user_id != Auth::User()->id)
                                <div class="cart-btn">
                                    <form id="demo-form2" method="post" action="{{ route('addtocart',['course_id' => $wish->courses->id, 'price' => $wish->courses->price, 'discount_price' => $wish->courses->discount_price ]) }}" data-parsley-validate class="form-horizontal form-label-left">
                                        {{ csrf_field() }}

                                        <input type="hidden" name="category_id" value="{{$wish->courses->category['id']}}" />


                                        <button type="submit" class="btn btn-primary wishlist-cart" title="Add To Cart"><i data-feather="shopping-cart" class="rgt-10"></i></button>
                                    </form>
                                </div>
                                @endif
                            </li>
                            <li class="protip-wish-btn-two">
                                <div class="wishlist-btn txt-rgt">
                                    <form method="post" action="{{url('delete/wishlist', $wish->id)}}" data-parsley-validate class="form-horizontal form-label-left">
                                        ...
                                        <button type="submit" class="btn btn-primary trash-icon" title="Remove From Wishlist"><i data-feather="trash" class="rgt-10"></i></button>
                                    </form>
                                </div>
                            </li>
                        </ul>
...
<section id="search-block" class="search-main-block search-block-no-result text-center">
    <div class="container-xl">
        <div class="no-result-img btm-20">
            <img src="{{ url('/images/no-result.png') }}" class="img-fluid" alt="">
        </div>
