		...
		<div class="cart-items btm-30">
	        <div class="row">
	        	<div class="col-lg-4 col-sm-4">
	        		<h4 class="cart-heading bg-white">{{ __('Your Items') }}
            		(@php
                        if(auth::check())
	        			{
	            		...
	            		@if(isset($one_course) && $one_course == 1)
	            			...
	            			<div class="row btm-10">
			            		...
			            		<div class="col-lg-9 col-8">
			            			<ul>
			            				@if($cart->id != NULL)
			            					<li class="checkout-course-title"><a href="{{ route('user.course.show',['id' => $cart->id, 'slug' => $cart->slug ]) }}">{{ str_limit($cart->title, $limit =35 , $end = '...') }}</a></li>
			            				@else
		                                ...
		                                @if($cart->discount_price == !NULL)
                                         ...
                                         <li>
                                         	<span><s>{{ activeCurrency()->getData()->position == 'l'  ? activeCurrency()->getData()->symbol :'' }}{{ price_format(  currency($course->price, $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}{{ activeCurrency()->getData()->position == 'r'  ? activeCurrency()->getData()->symbol :'' }}</s>
                                         	</span>
                                        </li>
			            				...
			            				@else
                                            ...
                                            <li>
                                            	<span>{{ activeCurrency()->getData()->position == 'l'  ? activeCurrency()->getData()->symbol :'' }}{{ price_format(  currency($course->price, $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}{{ activeCurrency()->getData()->position == 'r'  ? activeCurrency()->getData()->symbol :'' }}</span>
                                           	</li>
			            		...
			            		<div class="col-lg-9 col-8">
			            			<ul>
		                                ...
		                                @if($cart->offer_price == !NULL)
			            					...
			            					<li class="checkout-course-price"><b>
			            					{{ activeCurrency()->getData()->position == 'l'  ? $currency->symbol :'' }}
			            					{{ price_format(  currency($cart->offer_price, $from = $currency->code, $to = $currency->code, $format = false)) }}

			            					{{ activeCurrency()->getData()->position == 'r'  ? $currency->symbol :'' }}</b>  
			            					<s>
			            					{{ activeCurrency()->getData()->position == 'l'  ? $currency->symbol :'' }}
			            					{{ price_format(  currency($cart->price, $from = $currency->code, $to = $currency->code, $format = false)) }}
			            					{{ activeCurrency()->getData()->position == 'r'  ? $currency->symbol :'' }}</s></li>
			            					
			            				@else
			            					
			            					<li class="checkout-course-price"><b>{{ activeCurrency()->getData()->position == 'l'  ? $currency->symbol :'' }}{{ price_format( currency($cart->price, $from = $currency->code, $to = $currency->code, $format = false)) }}{{ activeCurrency()->getData()->position == 'r'  ? $currency->symbol :'' }}</b></li>
	            ...
	            <div class="col-lg-8 col-sm-8">
	            	...
	            	@php  
						...
						$string =  currency($cart_total, $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = true) ;
        			...
        			<div class="payment-gateways">
	            		<div id="accordion" class="second-accordion">
							...
							@if($gsetting->pesapal_enable == 1)
							<div class="card">
	                            <div class="card-header" id="headingPesapal">
							        <div class="panel-title">
							            <label for='rpesapal'>
							              <input type='radio' id='rpesapal' name='occupation' value='Working' required />
							              <a data-toggle="collapse" data-parent="#accordion" href="#collapsePesapal"></a>
							              <img src="{{ url('images/payment/pesapal.png') }}" class="img-fluid" alt="course">
							            </label>
							        </div>
						    	</div>
							    <div id="collapsePesapal" class="panel-collapse collapse in">
							        <div class="card-body">     
								        <form accept-charset="UTF-8" action="{{route('pesapal.pay')}}" method="POST"autocomplete="off" enctype="multipart/form-data">
								    		{{ csrf_field() }}
								    		<input type="hidden" name="amount"  value="{{ $mainpay }}" />
										    <button class='form-control btn btn-primary pesapalg' type='submit'>{{ __('Proceed') }}</button>
								        </form>	    
							        </div>
							    </div>
							</div>
											     ...
											     <div class="row">
											        <div class="col-md-12">
											            <div class="form-group">
											                ...
											                <input type="text" name="name" class="form-control" value="{{ Auth::user()->fname }}" placeholder="{{ __('Enter Name')}}" required>
											            </div>
											        </div>
											        <div class="col-md-12">
											            <div class="form-group">
		                            		...
		                            		<form action="{{ route('dopayment') }}" method="POST">
		                            			@csrf
		                         				...
		                         				<script
												    ...
												    data-amount= "{{ $mainpay*100 }}"
												    data-currency="{{ Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code }}"
												    data-order_id=""
		                            	...
		                            	<div class="payment-proceed-btn">
		                            		<form method="post" action="{{ url('/paywithpayment') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
		                            			@csrf
											        ...
											        <div class="col-md-12">
											            <div class="form-group">
											                <input type="hidden" name="amount" class="form-control" placeholder="" value="{{ Crypt::decrypt($secureamount) }}" readonly="">
											            </div>
											        </div>
											        <div class="col-md-12">
											            <button class="btn btn-primary" title="checkout" type="submit">{{ __('Proceed') }}</button>
