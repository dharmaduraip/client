  ...
  <style>
   ...
   /* @if($invoice['border_enable']=='1') .border-one {
    ...
    @endif*/
     @if($invoice['border_enable']=='1') .border-one {
    border: 5px;
    border-style:{{$invoice['border_style']}};
    border-color:{{ $invoice['border_color']}};
    background-color: var(--background-white-bg-color);
    margin-bottom: 40px;
    border-radius: {{$invoice['border_radius'] }}px
  }
  @endif
  </style>
  <!-- end theme styles -->
                        ...
                        <td style="text-align:left">
                          @if($invoice['logo_enable'] == '1')
                              @if($setting['logo_type'] == 'L')
                                  <img src="{{ public_path('images/logo/'.$setting['logo']) }}" class="img-fluid" width="150px" />
                              @else
                                  <a href="{{ url('/') }}"><b>
                        ...
                        <td style="text-align:right">
                            @if($invoice['logo_enable'] == '1')
                                @if($invoice->signature != NULL)
                                    <img src="{{ public_path('images/signature/'.$invoice['signature']) }}" class="img-fluid-invoice"
                ...
                <!-- <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="invoice-logo">
                      @if($invoice['logo_enable'] == '1')
                      @if($setting['logo_type'] == 'L')
                    <img src="{{ public_path('images/logo/'.$setting['logo']) }}" class="img-fluid" width="150px" alt="logo">
                      @else()
                      <a href="{{ url('/') }}"><b>
                          <div class="logotext">{{ $setting['project_title'] }}</div>
                        </b></a>
                  ...
                  <tr>
                    <td>
                      <div class="purchase-history-course-img">
                        ...
                        <a
                          href="{{ route('course.content',['id' => $orders->bundle->id, 'slug' => $orders->bundle->slug ]) }}"><img
                      ...
                      <div class="purchase-history-course-title">
                        <a
                          href="{{ route('course.content',['id' => $orders->bundle->id, 'slug' => $orders->bundle->slug ]) }}">{{ $coursess->title }}</a>
                      </div>
                    </td>
                  </tr>
                </tbody>
