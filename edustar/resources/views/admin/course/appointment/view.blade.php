...
<a href="{{ url('course/create/'. $appoint->course_id) }}" class="float-right btn btn-primary-rgba"><i class="feather icon-arrow-left mr-2"></i>{{ __('Back') }}</a>
</div>
@endslot
@endcomponent
          ...
          <form action="{{route('appointment.update',$appoint->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row ">
              <div class=" form-group col-md-6">
                <label for="exampleInputTit1e">{{ __('Accept') }}:</label><br>
                <label class="switch">
                  <input class="slider" type="checkbox" id="appoint_accept" name="accept" {{ $appoint->accept == 1 ? 'checked' : '' }}  />
                  <span class="knob"></span>
                </label> 
                  <!-- <input  type="checkbox" name="search_enable"  id="appoint_accept" {{ $appoint->accept == 1 ? 'checked' : '' }}  class="custom_toggle"/>            -->
              </div>
