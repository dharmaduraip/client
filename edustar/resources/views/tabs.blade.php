            ...
            <div class="item immi-slider-block development">
                <div class="genre-slide-image @if($gsetting['course_hover'] == 1) protip @endif" data-pt-placement="outside" data-pt-interactive="false" data-pt-title="#prime-next-item-description-block{{$c->id}}">
                    <div class="view-block">
                        ...
                        <div class="view-dtl">
                            <div class="view-heading"><a href="{{ route('user.course.show',['id' => $c->id, 'slug' => $c->slug ]) }}">{{ str_limit($c['title'], $limit = 35, $end = '...') }}</a></div>
                            <div class="user-name">
                                            ...
                                            @if(Auth::check())
                                                @if(Auth::User()->role == "admin")
                                                    <div class="protip-btn">
                                                        <a href="{{ route('course.content',['id' => $c->id, 'slug' => $c->slug ]) }}" class="btn btn-secondary" title="course">Go To Course</a>
                                                    </div>
                                                @else
                                                    @php
                                            ...
                                            @else
                                                <div class="protip-btn">
                                                    ...
                                                    <form id="demo-form2" method="post" action="{{ route('guest.addtocart', $c->id) }}"
                                                        ...
                                                        <div class="box-footer">
                                                         <button type="submit" class="btn btn-primary shopping-cart"><i data-feather="shopping-cart"></i>&nbsp;{{ __('Add To Cart') }}</button>
                                                        </div>
                                                    </form>

                                                    @else

                                                    <a href="{{ route('login') }}" class="btn btn-primary shopping-cart"><i data-feather="shopping-cart"></i>&nbsp;{{ __('Add To Cart') }}</a>
                                        ...
                                        @else
                                             @if(Auth::check())
                                                ...
                                                @else
                                                    ...
                                                    @if($enroll == NULL)
                                                        <div class="protip-btn">
                                                            <a href="{{url('enroll/show',$c->id)}}" class="btn btn-primary" title="Enroll Now">{{ __('Enroll Now') }}</a>
                                                        </div>
                                                    @else
                                                        <div class="protip-btn">
