...
redisClient.subscribe(['newRequest', 'checkTransportRequest', 'checkOrderRequest', 'checkServiceRequest','checkDeliveryRequest', 'providerUpdate', 'settingsUpdate', 'paymentUpdate']);
...
redisClient.on("message", function(channel, data) {
	...
	if(channel == 'checkDeliveryRequest')  {
		//Ride request
		if(data.type == "DELIVERY") {
			var room = `${data.room}_R${data.id}_${data.type}`;
			io.sockets.in(room).emit('deliveryRequest', { message: `New delivery request created in ${room}` } );
			var rooms = io.sockets.adapter;
		}
	}
