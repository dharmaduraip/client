...
class CreateStoresTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection('order')->create('stores', function (Blueprint $table) {
            ...
            $table->decimal('free_delivery_limit', 10, 2)->default(0);
            $table->string('currency')->nullable();
            $table->string('currency_symbol')->nullable();
            $table->double('wallet_balance', 10, 2)->default(0);
            $table->enum('is_veg', ['Pure Veg','Non Veg','Mixed'])->nullable();
            $table->Integer('store_response_time')->default(60);
            $table->Integer('bestseller')->default(5);
            $table->Integer('bestseller_month')->default(3);
