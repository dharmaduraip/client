...
class StoreOrder extends BaseModel
{
    ...
    protected $appends = ['created_time','assigned_time','delivery','pickup','delivery_time'];

    protected $fillable = [
        'id', 'admin_service', 'store_order_invoice_id', 'user_id', 'user_address_id', 'promocode_id', 'store_id', 'provider_id', 'provider_vehicle_id', 'company_id', 'city_id', 'country_id', 'note', 'description', 'route_key', 'delivery_date', 'schedule_datetime', 'pickup_address', 'delivery_address', 'order_type', 'order_otp', 'order_ready_time', 'order_ready_status', 'paid', 'user_rated', 'provider_rated', 'cancelled_by', 'cancel_reason', 'currency', 'status', 'schedule_status', 'assigned_at', 'timezone', 'request_type','productId','package_description','number_of_packages','collectable_delivery_cost'      
    ];
    ...
    {
       return $this->belongsTo('App\Models\Common\Promocode');

    }

   	public function store()
   	{
       return $this->hasOne('App\Models\Order\Store','id','store_id')->select('store_name','id','latitude','longitude','store_type_id','estimated_delivery_time', 'flat_no', 'street', 'store_location', 'store_response_time');
   	}
    ...
    {
        return $this->belongsTo('App\Models\Common\Reason', 'cancel_reason');
    }
    ...
    {
        return $this->hasOne('App\Models\Common\Rating', 'request_id')->where('admin_service','ORDER');
    }
    ...
    public function scopeHistoryUserTrips($query, $user_id,$showType='')
    {
        ...
        if($showType == 'past'){
            ...
            $history_status = array('ORDERED','RECEIVED','STORECANCELLED','SEARCHING','ACCEPTED','STARTED','ARRIVED','PICKEDUP','REACHED','DROPPED');
        }
