...
class Store extends BaseModel implements JWTSubject, AuthenticatableContract, AuthorizableContract
{
    ...
    protected $fillable = [
        'store_type_id','store_name','email','store_location', 'latitude','jwt_token', 'longitude', 'store_zipcode', 'country_id', 'city_id', 'zone_id', 'contact_person', 'contact_number', 'country_code', 'picture', 'store_packing_charges', 'store_gst', 'commission', 'offer_min_amount', 'offer_percent', 'estimated_delivery_time', 'description', 'currency_symbol', 'free_delivery', 'is_veg', 'rating', 'is_bankdetail', 'company_id'
    ];

    protected $hidden = [
     	'password','company_id','created_type','jwt_token', 'created_by', 'modified_type', 'modified_by', 'deleted_type', 'deleted_by', 'created_at', 'updated_at', 'deleted_at'
     ]; 
    ...
    public function orderInvoices()
    {
        ...
        $dateS = \Carbon\Carbon::now()->startOfMonth()->subMonth($this->bestseller_month);
        $dateE = \Carbon\Carbon::now(); 

       return $this->hasmany('App\Models\Order\StoreOrderInvoice', 'store_id','id')->whereBetween('created_at',[$dateS,$dateE]);
    }
