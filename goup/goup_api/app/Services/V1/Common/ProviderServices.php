...
class ProviderServices { 
	...
	public function checkRequest($request) {
		...
		try{
			...
			$provider = Provider::where('id', $this->user->id)->where('wallet_balance' ,'>=', $siteConfig->provider_negative_balance)->first();
			$IncomingRequests = [];
			if($provider != null) {
			...
			if(!empty($IncomingRequests)){
				...
				for ($i=0; $i < count($IncomingRequests); $i++) {
					...
					elseif($admin_service == "DELIVERY"){
						$config = $settings->delivery;
		...
		try {        
			...
			try {
				if($request->admin_service == "TRANSPORT" ) {
                    ...
                    $provider_service= ProviderService::where('provider_id', $this->user->id)->where('provider_vehicle_id', $provider_vehicle->id)->first();
			...
			try {
				if($request->admin_service == "DELIVERY" ) {
					...
					try{
						...
						$newRequest = \App\Models\Delivery\DeliveryRequest::with('delivery','deliveries','service','deliveries.package_type','delivery.package_type')->where('id', $request->id)->first();
						$delivery = \App\Models\Delivery\Delivery::with('user')->where('delivery_request_id', $newRequest->id)->get();
						foreach ($delivery as $key => $value) {

							$value->provider_id = $this->user->id;
							$value->save();
						}
			...
			if($request->admin_service == "TRANSPORT") {
                ...
                $newRequest->provider_service_id = isset($provider_service->id)?$provider_service->id:NULL;
            }
			...
			if($newRequest->schedule_at != ""){
				...
				try {
					...
					if($request->admin_service == "DELIVERY" ) {
						$CheckScheduling = \App\Models\Delivery\DeliveryRequest::where('status','SCHEDULED')
							->where('provider_id', $this->user->id)
							->whereBetween('schedule_at',[$beforeschedule_time,$afterschedule_time])
							->count();
				...
				if($request->admin_service == "SERVICE" ) {
					...
					//Send message to socket
					$requestData = ['type' => 'DELIVERY', 'room' => 'room_'.$this->company_id, 'id' => $newRequest->id, 'city' => ($this->settings->demo_mode == 0) ? $newRequest->city_id : 0, 'user' => $newRequest->user_id, 'message' => 'testing' ];
				}else{
					$newRequest->status = "STARTED";
					$newRequest->save();    
					//Send message to socket
				...
				if($newRequest->admin_service == "DELIVERY") $publishUrl = 'checkDeliveryRequest';
				...
				if(count($Providers) > 0) {
					foreach ($Providers as $key => $Provider) {
						...
						if($this->settings->transport->broadcast_request == 1){
						   (new SendPushNotification)->IncomingRequest($Provider->id, $admin_service); 
						}
	...
	public function cancel($id, $admin_service) {
		...
		} else if($admin_service == "DELIVERY") {
			try {
				$newRequest =  \App\Models\Delivery\DeliveryRequest::findOrFail($id);
				$newRequest->status = 'CANCELLED';
				$newRequest->save();

				$requestData = ['type' => 'DELIVERY', 'room' => 'room_'.$this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => ($this->settings->demo_mode == 0) ? $newRequest->country_id : 0 ];
				app('redis')->publish('newRequest', json_encode( $requestData ));
	...
	public function assignNextProvider($request_id, $admin_service) 
	{
		...
		if($admin_service == "TRANSPORT" ) {
			...
			try {
				$newRequest = \App\Models\Delivery\DeliveryRequest::with('user')->find($UserRequest->request_id);

				$setting = Setting::where('company_id', $newRequest->company_id)->first();

				$requestData = ['type' => 'DELIVERY', 'room' => 'room_'.$this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => ($setting->demo_mode == 0) ? $newRequest->country_id : 0 ];
		...
		try {
			...
			if($admin_service == "TRANSPORT" ) {
				...
				try {
					\App\Models\Delivery\DeliveryRequest::where('id', $newRequest->id)->update(['status' => 'CANCELLED']);
				} catch(\Throwable $e) { }
			}
	...
	public function providerHistory(Request $request, $UserRequest, $callback) 
	{
         ...
         try{
        	...
        	if($request->has('search_text') && $request->search_text != null) {
               $UserRequest->ProviderhistroySearch($request->search_text);
	        }  
