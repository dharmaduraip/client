		...
		try {        
			...
			try {
				if($request->admin_service == "TRANSPORT" ) {
					...
					$provider_vehicle = ProviderVehicle::where('provider_id', $this->user->id)->where('vehicle_service_id', $newRequest->ride_delivery_id)->where('admin_service', $request->admin_service)->first();
			...
			if($request->admin_service == "TRANSPORT") {
                ...
                $newRequest->provider_service_id = $provider_service->id;
            }
		...
		try {
			...
			try {
				...
				if($request->admin_service == "ORDER" ) {
					...
					$ride_request = \App\Models\Transport\RideRequest::find($request->id);
					if($config->broadcast_request != 1 || ($ride_request->type == 'AIRPORT' && $request->admin_service == "TRANSPORT"))
			        {
	...
	public function assignProvider($config, $IncomingRequest) {
		if($config !='' && $config->manual_request == 0){
			...
			if($IncomingRequest->request->status == 'SEARCHING' && $IncomingRequest->request->time_left_to_respond < 0) {
				...
				if($IncomingRequest->request->request->admin_service == "TRANSPORT" && $IncomingRequest->request->request->type == 'AIRPORT'){
