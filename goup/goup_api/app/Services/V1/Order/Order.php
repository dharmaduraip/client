...
class Order {
    ...
    */
    ...
    public function shopAccept(Request $request) {
        try {
            if ($request->store_type_id != '6') {
                ...
                $storeorder =StoreOrder::findorfail($request->store_order_id);
                $storeorder->store_id =$request->store_id;
                $storeorder->status='RECEIVED';
                $storeorder->save();
                $userRequest = UserRequest::where('request_id',$request->store_order_id)->first();
                $userRequest->status = "ORDERED";
                $userRequest->save();
                $store =Store::findorfail($request->store_id);
                $user = User::findorfail($request->user_id);
                $smsMessage ='your request has been accepted by  '.$store->store_name;
                $plusCodeMobileNumber =  $user->mobile;
                $company_id=base64_decode('MQ==');
                $send_sms = Helper::send_sms($company_id, '+'.$user->country_code.''.$user->mobile, 'Your request has been accepted by  ' . $store->store_name." The location to your shop is ". "https://addisplus.net/locationpoint/".$storeorder->latitude."/".$storeorder->longitude."/".$store->latitude."/".$store->longitude. ' The shop address is ' . $store->	store_location. ' for further details contact shop'. $store->contact_number." Please collect your product on or before ".$storeorder->schedule_datetime);
                return ['status' => 200, 'message' => 'sms sent successfully'];
            }
        }  catch (\Throwable $e) {
            \Log::info($e);
            return ['status' => 500, 'message' => trans('admin.something_wrong'), 'error' => $e->getMessage()];
        }
    }
    ...
    public function returnOrders(Request $request) {
        $orderRequest = StoreOrder::findOrFail($request->id);
        $orderRequest->status = 'RETURNED';   
        $orderRequest->cancel_reason = $request->cancelreason;     
        $orderRequest->save();
        (new UserServices())->cancelRequest($orderRequest);
        $user = User::findOrFail($orderRequest->user_id);
        // (new SendPushNotification)->orderShopreturned($orderRequest, 'order', 'Order Accepted');
        $company_id=base64_decode('MQ==');
        $send_sms = Helper::send_sms($company_id, '+'.$user->country_code.''.$user->mobile, 'The shop have returned the product to the admin for further details contact admin.');
        // $storeorderstatus =  new StoreOrderStatus;
        // $storeorderstatus->store_order_id=$request->id;
        // $storeorderstatus->status='CANCELLED';
        // $storeorderstatus->company_id=Auth::guard('shop')->user()->company_id;
        ...
        // $requestData = ['type' => 'ORDER', 'room' => 'room_'.Auth::guard('shop')->user()->company_id, 'id' => $request->id,'shop'=> $request->store_id, 'user' => $request->user_id ];
        // app('redis')->publish('checkOrderRequest', json_encode( $requestData ));
        $data=['status' => 200, 'message' =>'Returned succcesfully'];
        return $data;
    }
    ...
    public function collectedOrder(Request $request) {
        ...
        $company_id=base64_decode('MQ==');
        try {
            $send_sms = Helper::send_sms($company_id, '+'.$user->country_code.''.$user->mobile, 'You have collected the product successfully.');
            \Log::info($send_sms);
        } catch (\Throwable $th) {
            \Log::info($th);
        }
        ...
        // $requestData = ['type' => 'ORDER', 'room' => 'room_'.Auth::guard('shop')->user()->company_id, 'id' => $request->id,'shop'=> $request->store_id, 'user' => $request->user_id ];
        // app('redis')->publish('checkOrderRequest', json_encode( $requestData ));
        $data=['status' => 200, 'message' =>'Collected succcesfully'];
        return $data;
    }

    public function qrscanned(Request $request) {
        $orderRequest = StoreOrder::findOrFail($request->id);
        if ($orderRequest->productId ==  $request->productId) {
            (new SendPushNotification)->qrScanned($orderRequest, 'ORDER');
            $orderRequest->qrCode = 1;   
            $orderRequest->save();
            $data=['status' => 200, 'message' =>'Verified succcesfully'];
        }
        else{
            $data=['status' => 422, 'message' =>'Wrong product Scanned'];
        }
