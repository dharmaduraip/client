...
class Ride { 
	...
	*/
	...
	public function createRide(Request $request) {
		...
		$ride_city_price = RideCityPrice::where('geofence_id',$geofence['id'])->where('ride_delivery_vehicle_id', $request->service_type)->first();
		...
		try {
			...
			\Log::info('unit mearsure:'.$siteConfig->unit_measurement);
			$rideRequest->unit = isset($siteConfig->unit_measurement) ? $siteConfig->unit_measurement : 'Miles';
...
\Log::info('unit measurements:'. $siteConfig->unit_measurement);
	...
	public function invoice($request_id, $toll_price = 0)
	{
		//try {                      
			...
			$Commision =  (($Base_price-$Discount)+$peakamount+$total_waiting_amount + $Tax) * ( $commission_percentage/100 );

			$Base_price += $Commision;
			$Fixed += $Commision;

			$Total = ($Base_price-$Discount) + $Tax;
			// $Total = ($Base_price-$Discount) + $Tax + $Commision;
