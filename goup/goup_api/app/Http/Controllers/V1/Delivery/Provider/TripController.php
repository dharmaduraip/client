...
class TripController extends Controller
{
    ...
    public function index(Request $request)
	{
		try{
			...
			$IncomingRequests = DeliveryRequest::with(['delivery.payment','chat','user','delivery'=> function($q) {
				$q->where('status', '!=' ,'COMPLETED');
			},'deliveries.payment','deliveries'=> function($q) {
				$q->where('status', '!=' ,'COMPLETED');
			},'service'])
...
//return $IncomingRequests;
	...
	public function trips(Request $request)
	{
		try{
			...
			$withCallback=[
							...
							'payment' => function($query){  $query->select('request_id','total','cash','card','payment_mode','payable'); }, 
		    ...
		    $ProviderRequests = DeliveryRequest::select('id', 'booking_id', 'assigned_at', 's_address', 'd_address','provider_id','user_id','timezone','delivery_vehicle_id', 'status', 'cancelled_by', 'cancel_reason', 'provider_vehicle_id','started_at','total_amount')->with('deliveries.payment');
	...
	public function gettripdetails(Request $request,$id)
	{
		try{
			...
			$jsonResponse = [];
			$jsonResponse['type'] = 'delivery';
			$providerrequest = DeliveryRequest::with(array('payment','deliveries.payment','service','user','service_type',
