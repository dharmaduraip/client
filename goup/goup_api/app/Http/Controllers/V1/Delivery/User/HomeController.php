...
use Actions;
    ...
    public function trips(Request $request) {
        try{
             ...
             $userrequest=DeliveryRequest::select('id', 'booking_id', 'assigned_at', 's_address', 'd_address','provider_id','user_id','timezone','delivery_vehicle_id', 'status', 'cancelled_by', 'cancel_reason', 'user_rated', 'provider_rated','payment_mode', 'provider_vehicle_id','created_at','schedule_at','total_amount')->with('deliveries.payment');
	...
	public function gettripdetails(Request $request,$id) {
		try{
			...
			$jsonResponse = [];
			$jsonResponse['type'] ='delivery';
			$userrequest = DeliveryRequest::with(['provider','deliveries.package_type','deliveries.payment','service_type','service','provider_vehicle','dispute'=> function($query){ 
	...
	{
		$dispute = Dispute::select('id','dispute_name','service')->where('service','DELIVERY')->where('dispute_type','user')->where('status','active')->get();
