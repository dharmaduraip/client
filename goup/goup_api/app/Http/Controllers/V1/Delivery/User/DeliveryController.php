...
class DeliveryController extends Controller
{
	...
	{
		$delivery_types = DeliveryType::where('delivery_category_id', $category_id)->where('status',1)->where('company_id', $this->company_id)->orderby('delivery_name')->get();
	...
	public function services(Request $request)
	{
		...
		$delivery->promocodes = [];
		$promocodes = Promocode::where('company_id', $this->company_id)->where('service', 'DELIVERY')
					->where('expiration','>=',date("Y-m-d H:i"))
					->withCount('promousage')
					// ->having('promousage_count','<',2)
					->wherehas('promousage', function($query) {
						$query->where('user_id', Auth::guard('user')->user()->id);
					})
					->get();

					foreach($promocodes as $key=>$promocode) {
						if($promocode->promousage_count < $promocode->limit_per_user){
							$delivery->promocodes[$key] = $promocode;
						}
					}
				} else {
					$delivery->promocodes = [];
	...
	public function estimate(Request $request)
	{
		...
		if($request->has('receiver_instruction') && $request->receiver_instruction != "") {
			foreach ($request->receiver_instruction as $weight) {
				if(5000 < $weight) return Helper::getResponse(['status' => 422, 'message' => 'Instruction limit should not exceed more than 5000']);
			}
		}
		...
		if( Auth::guard(strtolower(Helper::getGuard()))->user() != null ) {
			$promocodes = Promocode::where('company_id', $this->company_id)->where('service', 'DELIVERY')
					...
					foreach($promocodes as $key=>$promocode) {
						if(count($promocode->promousage) < $promocode->limit_per_user){
							$promocod[] = $promocode;
						}
					}
		...
		return Helper::getResponse(['data' => ['fare' => $fare, 'service' => $service, 'promocodes' => $promocod, 'currency' => $currency]]);
	}
	...
	public function status(Request $request)
	{
		...
		try{
            ...
            if(!empty($deliveryRequest)){
                //$deliveryRequest[0]['delivery_otp'] = (int) $this->settings->delivery->otp ? $this->settings->delivery->otp : 0 ;
	...
	public function checkDelivery(Request $request, $id)
	{
		...
		try{
			...
			$peak_percentage = 1+(0/100)."X";
			$peak = 0;
			\Log::info('delivery request table la iruka');
			\Log::info($rideRequest[0]);
			/*$start_time_check = PeakHour::where('start_time', '<=', $start_time)->where('end_time', '>=', $start_time)->where('company_id', '>=', Auth::guard('user')->user()->company_id)->first();

			if( count($rideRequest) > 0 && $start_time_check){

				$Peakcharges = DeliveryPeakPrice::where('delivery_city_price_id', $rideRequest[0]['city_id'])->where(' delivery_vehicle_id ', $rideRequest[0]['ride_delivery_id'])->where('peak_hour_id',$start_time_check->id)->first();
			...
			$search_status = ['SEARCHING','SCHEDULED'];
			$rideRequestFilter = Delivery::DeliveryRequestAssignProvider(Auth::guard('user')->user()->id,$search_status)->get(); 
			...
			if(!empty($rideRequest)){
				...
				if(count($delivery_payments)) {
					...
					foreach ($delivery_payments as $key => $delivery_payment) {
						...
						$weight_fare =+ $delivery_payment->weight_fare;
	...
	public function searchRideDispute(Request $request)
	{
		...
		foreach ($queries as $query) {
			$RideRequestDispute = DeliveryRequestDispute::where('delivery_request_id', $query->id)->first();
	...
	public function requestStatementHistory(Request $request)
	{
		try {
			...
			foreach ($dataval as $ride) {
				...
				$rideid  = $ride->id;
				$earnings = DeliveryPayment::select('total')->where('delivery_id', $rideid)->where('company_id',  Auth::user()->company_id)->first();
				if ($earnings != null) {
	...
	{
		try {
			$data = DeliveryRequest::with('user','deliveries.payment', 'provider', 'rating', 'payment')->findOrFail($id);
	...
	public function payment(Request $request)
	{
		...
		//try {
			...
			// $UserRequest = Delivery::find($request->id);

			// $delivery = DeliveryRequest::findOrFail($UserRequest->delivery_request_id);
			
			// $UserRequest = DeliveryRequest::find($request->id);

			// $delivery = Delivery::findOrFail($UserRequest->id);
			...
			if($paymentMode == 'REVOLUT'){
	            ...
	            $response = $this->revoult_order($this->client_revoult,$totalAmount,$random);
			...
			if($paymentMode != 'CASH') {
				...
				if ($totalAmount == 0) {
					...
					\Log::info('user:'.$UserRequest->user_id);
					$log = new PaymentLog();
					$log->company_id = $this->company_id;
					$log->admin_service = $UserRequest->admin_service;
					$log->user_type = 'user';
					...
					switch ($paymentMode) {
						...
						case 'RAZORPAY':
						    ...
						    $gateway = new PaymentGateway('razorpay');
