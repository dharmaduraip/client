...
class AllStatementController extends Controller
{
    ...
    public function statement_provider(Request $request)
    {
		...
		foreach($Providers as $index => $Provider){
            ...
            if(in_array('DELIVERY',$servicenames) == TRUE){
                $Deliveries = \App\Models\Delivery\DeliveryRequest::where('provider_id',$Provider->id)
                            ->where('status','<>','CANCELLED')
                            ->get()->pluck('id');
                $Providers[$index]->delivery_count = $Deliveries->count();
                $deliveryPaymentQ = \App\Models\Delivery\DeliveryPayment::whereIn('delivery_id', $Deliveries)
                                ->select(\DB::raw('SUM(ROUND(provider_pay)) as overall'))->first();
                if($deliveryPaymentQ != null){
                    $deliveryPayment = $deliveryPaymentQ->overall;
                }
            }
            ...
            $Providers[$index]->payment = $ridePayment +  $servicePayment + $orderPayment + $deliveryPayment;
        }
   ...
   public function statement_user(Request $request)
   {
	...
	try{
        ...
        foreach($Users as $index => $User){
            ...
            if(in_array('DELIVERY',$servicenames) == TRUE){
                $Deliveries = \App\Models\Delivery\DeliveryRequest::where('user_id',$User->id)
                            ->where('status','<>','CANCELLED')
                            ->where('status','<>','SCHEDULED')
                            ->get()->pluck('id');
                $Users[$index]->delivery_count = $Deliveries->count();
                $deliveryPaymentQ = \App\Models\Delivery\DeliveryPayment::whereIn('delivery_id', $Deliveries)
                                ->select(\DB::raw('SUM(ROUND(total)) as overall'))->first();
                if($deliveryPaymentQ != null){
                    $deliveryPayment = $deliveryPaymentQ->overall;
                }
            }
            ...
            $Users[$index]->payment = $ridePayment +  $servicePayment + $orderPayment + $deliveryPayment;           
        }
    ...
    public function statement_admin(Request $request)
    {
      ...
      public function statement_fleet(Request $request)
    {

        try{        
        $datum = FleetWallet::whereHas("provider",function($query) use ($request){
             $query->where('country_id',$request->country_id);
        })->select('*','created_at as dated')->where('company_id', Auth::user()->company_id);
        if($request->has('search_text') && $request->search_text != null) {
            $datum->Search($request->search_text);
        }
        if($request->has('order_by')) {
    ...
    public function getData(Request $request)
    {
        ...
        try{        
            ...
            if($request->service_type=='DELIVERY'){
                $ride_type = \App\Models\Delivery\DeliveryType::where('company_id', Auth::user()->company_id)->select('id','delivery_name')->get();
            }
