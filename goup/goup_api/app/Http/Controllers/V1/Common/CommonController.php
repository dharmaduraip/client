...
class CommonController extends Controller
{
    ...
    public function base(Request $request) {
        ...
        if ($license != null) {
            try{  
                ...
                $appsettings=[];
                if(isset($settings) && count((array)$settings)>0){
    ...
    public function admin_services() {
        $admin_service = AdminService::where('company_id', Auth::user()->company_id)->whereNotIn('admin_service', ['Deliveries'] )->where('status', 1)->get();
