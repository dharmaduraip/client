...
class SocialLoginController extends Controller
{
	...
	try{	
		...
		if($provider == 'user')  {
			...
			if($user){
				if($request->has('device_type')) $user->device_type = $request->device_type;
				...
				if($request->has('device_type')) $newUser->device_type = $request->device_type;
				if($request->has('device_token')) $newUser->device_token = $request->device_token;
				$newUser->save();
