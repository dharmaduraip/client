...
class OrderController extends Controller
{
	...
	public function callTransaction($store_order_id){  
		...
		if($StoreOrder->paid==1){
			...
			$credit_amount=$paymentsStore->item_price + $paymentsStore->tax_amount-$paymentsStore->commision_amount;	
			// $credit_amount=$paymentsStore->total_amount-$paymentsStore->delivery_amount;	

		if($paymentsStore->payment_mode == 'CASH' && $paymentsStore->use_wallet == 0) {
			$transation['transaction_desc']='Order amount received from the user to provider';
			$transation['type']='D';
			$transation['id']=$StoreOrder->provider_id;
			$transation['amount']= -1 * ($paymentsStore->total_amount - $paymentsStore->delivery_amount);
			$this->createProviderWallet($transation);
			   
			$transation['transaction_desc']='Order amount received from the provider to admin';
			$transation['type']='C';
			$transation['transaction_type']=5;
			$transation['id']=$StoreOrder->provider_id;
			$transation['amount']= +1 * ($paymentsStore->total_amount - $paymentsStore->delivery_amount);
			$this->createAdminWallet($transation);
			$transation['transaction_desc']='Order commission received from the shop to admin';
			$transation['type']='C';
			$transation['transaction_type']=1;
			$transation['id']=$StoreOrder->provider_id;
			$transation['store_id']=$StoreOrder->store_id;

			$commisionAmount = $paymentsStore->commision_amount;
			$DiscountAmount = $paymentsStore->promocode_amount;



			if($commisionAmount >= $DiscountAmount) {
				$commisionAmount -= $DiscountAmount;
              if($DiscountAmount > 0){
				$transation['transaction_desc']='Promocode Discount amount debited from admin commission';
				$transation['type']='D';
				$transation['transaction_type']=10;
				$transaction['company_id'] = $StoreOrder->company_id;
				$transaction['admin_service'] = $StoreOrder->admin_service;
				$transaction['country_id'] = $StoreOrder->country_id;
				$transaction['transaction_id'] = $StoreOrder->id;
				$transaction['transaction_alias'] = $StoreOrder->store_order_invoice_id;
				$transation['amount']= -1 * ($DiscountAmount);
				$this->createAdminWallet($transation);
			  }
			} else {
				$commisionAmount -= $DiscountAmount;

				$transation['transaction_desc']='Promocode Discount amount recevied from admin account';
		        $transation['type']='C';
		        $transation['amount']= (1) * abs($commisionAmount);
		        $transation['id']=$StoreOrder->store_id;
			    $this->createShopWallet($transation);
			}

			$transation['transaction_desc']='Promocode Remaining Discount amount Credited';
				$transation['type']='C';
				$transation['transaction_type']=10;
				$transaction['company_id'] = $StoreOrder->company_id;
				$transaction['admin_service'] = $StoreOrder->admin_service;
				...
				$transaction['transaction_id'] = $StoreOrder->id;
				$transaction['transaction_alias'] = $StoreOrder->store_order_invoice_id;
			$transation['amount']= +1 * ($commisionAmount);
			$this->createAdminWallet($transation);
		} else {
			...
			if(!empty($paymentsStore->total_amount)){
        		...
        		//add the total amount to admin
			   	//$this->adminCredit($transation);
			   	$transation['transaction_desc']='Total amount received from the order from user to admin';
				$transation['transaction_type']=1;
				$transation['type']='C';        
				$this->createAdminWallet($transation);
			}
			...
			if(!empty($paymentsStore->delivery_amount)){
				...
				if($StoreOrder->order_type=='DELIVERY'){
					...
					$ad_det_amt= -1 * abs($transation['amount']);                  
			        $transation['transaction_desc']='Order amount sent to provider';
			        $transation['transaction_type']=9;
			        $transation['type']='D';
			        $transation['amount']=$ad_det_amt;
			        $this->createAdminWallet($transation);

			        $transation['transaction_desc']='Order amount received from the admin';
			        $transation['type']='C';
			        $transation['amount']=$transation['amount'];
			        $this->createProviderWallet($transation);
		...
		\Log::info('credit amount');
			...
			if($credit_amount>0){
				...
				$ad_det_amt= -1 * abs($transation['amount']);                  
		        $transation['transaction_desc']='Order amount sent to shop account';
		        $transation['transaction_type']=9;
		        $transation['type']='D';
		        $transation['amount']=$ad_det_amt;
		        $this->createAdminWallet($transation);
