...
class ServeController extends Controller 
{
	...
	public function updateServe(Request $request)
	{
		...
		try{
	        ...
	        $siteConfig = $settings->site;
			$serviceConfig = $settings->service;
			$serveRequest = ServiceRequest::with('user','service','service.serviceCategory','promocode')->findOrFail($request->id);
	...
	public function invoice($admin_service, $request_id,$extracharges,$extracharges_notes,$userdistance ='') 
    {
        try {
			...
			$Total = ($Fixed + $extracharges + $taxAmount + $commissionAmount);
	...
	public function historyList(Request $request)
	{
		try{
			...
			$withCallback=array('payment'=>function($query){
								$query->select('id','service_request_id','total','round_of','cash','card','tax','commision','payment_mode','payable');
			...
			$ProviderRequests = ServiceRequest::
							select('id','booking_id','user_id','provider_id','service_id','company_id','s_address','started_at','status','assigned_at','timezone','created_at');
	...
	public function getServiceHistorydetails(Request $request,$id)
	{
		try{
			...
			$providerrequest = ServiceRequest::with(array('payment'=>function($query){
				$query->select('id','service_request_id','total','round_of','payment_mode','fixed','tax','minute','extra_charges','total','commision','tips','payable','wallet','discount','cash','card');
			},'service'=>function($query){
				$query->select('id','service_category_id','service_name');
