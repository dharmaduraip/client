...
class ServiceController extends Controller
{
		...
		try{
			...
			if(!empty($serviceRequest)){
				...
				foreach($serviceRequest as $key=>$requestlist){
					...
					$requestlist->reasons =Reason::where('type','USER')->where('service','SERVICE')->where('status','Active')->get();
			...
			return Helper::getResponse(['data' => [
				...
				'data' => $serviceRequest, 
				'serve_otp' =>  isset($serviceConfig->serve_otp) ? $serviceConfig->serve_otp : 0, 
	...
	public function update_payment_method(Request $request)
	{
		...
		try{
			...
			(new SendPushNotification)->updateProviderStatus($user_request->provider_id, 'provider', trans('api.service.payment_updated'). ' : '. $request->payment_mode, '' ); 
	...
	public function rate(Request $request) 
	{
	  ...
	  public function payment(Request $request) {
		 ...
		 try {
			  $tip_amount = 0;
			  $serviceRequest = \App\Models\Service\ServiceRequest::with('user','promocode','service','service.serviceCategory')->find($request->id);
			...
			if($paymentMode == 'REVOLUT'){
	            ...
	            $log = new PaymentLog();
	            $log->user_type = 'user';
	            $log->admin_service = $serviceRequest->admin_service;
			  ...
			  if($paymentMode == 'NOWPAYMENT'){
	           ...
	           // $response = $this->nowpayment_estimate($this->client, $this->api_key);
	            ...
	            $response = $this->nowpayment_invoice($paymentMode,$serviceRequest, $this->client, $totalAmount, $random);
				  ...
				  if ($totalAmount == 0) {
					  ...
					  if($response->status == "SUCCESS") {  
						  ...
						  if( !empty($siteConfig->send_email) && $siteConfig->send_email == 1) {
								...
								if($serviceRequest->paid == 1){

									if($payment != null){

										dispatch(new SendServiceInvoiceEmailJob($serviceRequest, $payment, $serviceRequest->user->email));
									}
								} 
			      		  }
	...
	{
		try {
			$data = ServiceRequest::with('payment','user', 'provider','rating','service','serviceCategory')->findOrFail($id);
			return Helper::getResponse(['data' => $data]);
		} catch (\Throwable $e) {
			return Helper::getResponse(['status' => 404,'message' => trans('admin.something_wrong'), 'error' => $e->getMessage()]);
		}
	...
	public function nowpayment_invoice($paymentMode,$serviceRequest, $client, $pay_amount, $random)
    {
        \Log::info('payment amount:'.$pay_amount);
            $payload = [
                "form_params" => [
                    ...
                    "order_id" => $random,
                    "order_description" => "Service flow payment",
                    "success_url" => "https://easyjek.com/nowpayment/success?order_id=" . $random . "&payment_mode=" . $paymentMode . "&user_type=user" . "&admin_service=".$serviceRequest->admin_service,
