...
use Actions;
	...
	public function gettripdetails(Request $request,$id) {
		try{
			...
			$jsonResponse = [];
			$jsonResponse['type'] ='transport';
			$userrequest = RideRequest::with(['provider','payment','payment.promoCode','service_type','ride','provider_vehicle','dispute'=> function($query){  
			...
			if($data->payment)
			{
				...
				$data->payment->time_fare_text = 'Ride Time for '.$data->travel_time.' mins';	
