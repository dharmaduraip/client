...
class TripController extends Controller
{
    ...
    public function index(Request $request)
	{
		...
		try{
			...
			if($IncomingRequests != null) {
				...
				$total_distance = $IncomingRequests->total_distance;
				$total = $total_distance - $base_distance;
				if(!empty($IncomingRequests['payment'])) {
					$IncomingRequests['payment']['base_fare_text'] = 'Base Fare for '.$base_distance .' kms';
					$IncomingRequests['payment']['distance_fare_text'] = 'Distance Fare for '.$total.' Kms';
					$IncomingRequests['payment']['time_fare_text'] = 'Ride Time for '.$IncomingRequests->travel_time.' mins';

					$IncomingRequests['payment']['waiting_fare_text'] = 'Waiting Fare for '.$IncomingRequests['payment']['total_waiting_time'] .' mins';
					$IncomingRequests['payment']['discount_fare_text'] = 'Discount ('.$IncomingRequests['payment']['discount_percent'] .'%)';
				}
			}
	...
	public function accept_ride(Request $request)
	{
		...
		if($newRequest) {
			$provider_vehicle = ProviderVehicle::where('provider_id', $this->user->id)->where('vehicle_service_id', $newRequest->ride_delivery_id)->where('admin_service', 'TRANSPORT')->first();
	...
	public function gettripdetails(Request $request,$id)
	{
		try{
			...
			if($data->payment)
			{
				$total = $data->total_distance - $data->base_distance;
				$data->payment->base_fare_text = 'Base Fare for '.$data->base_distance .' '.$data->unit;
				$data->payment->distance_fare_text = 'Distance Fare for '.$total.' '.$data->unit;
				$data->payment->time_fare_text = 'Ride Time for '.$data->travel_time.' '.$data->unit;	
				$data->payment->waiting_fare_text = 'Waiting Fare for '.$data->payment->total_waiting_time.' mins';	
				$data->payment->discount_fare_text = 'Discount ('.$data->payment->discount_percent.'%)';
			}
            $jsonResponse['transport'] = $data;
			return Helper::getResponse(['data' => $jsonResponse]);
		}
	...
	public function get_ride_request_dispute(Request $request,$id) {
		...
		{
	    	$ride_request_dispute->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ride_request_dispute->created_at, 'UTC'))->setTimezone($ride_request_dispute->request->timezone)->format(Helper::dateFormat());	
	...
	public function callTransaction($request_id){  
		...
		if($UserRequest->paid==1){
			...
			if($paymentsRequest->is_partial==1){
				...
				if($paymentsRequest->payment_mode=="CARD" || $paymentsRequest->payment_mode=="REVOLUT" || $paymentsRequest->payment_mode=="NOWPAYMENT" || $paymentsRequest->payment_mode=="PAYPAL" ||  $paymentsRequest->payment_id=="WALLET"){
					$credit_amount=$paymentsRequest->total + $paymentsRequest->tips;
				}
				else{
