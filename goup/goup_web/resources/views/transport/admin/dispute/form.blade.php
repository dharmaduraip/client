...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            @if(empty($id))
                @php($action_text=__('transport.admin.dispute.add'))
            @else
                @php($action_text=__('transport.admin.dispute.edit'))
            @endif    
...
$('#namesearch').autocomplete({
    source: function(request, response) {
    	var url= getBaseUrl() +"/admin/usersearch";
    	sflag=0;
    	if($("#dispute_type").val()=='provider'){
    		sflag=1;
    		url= getBaseUrl() +"/admin/userprovider";
    	}
	    $.ajax
	    ({
	        type: "GET",
	...
	select: function (event, ui) {  
		...
		$.ajax({
			url:  getBaseUrl() +"/admin/disputeridesearch",
			type: 'post',
			headers: {
                "Authorization": "Bearer " + getToken("admin")
            },
...
function get_disputes(dispute_type){
	$.ajax({
		url:  getBaseUrl() +"/admin/disputelist",
		type: 'get',
		headers: {
                "Authorization": "Bearer " + getToken("admin")
            },
