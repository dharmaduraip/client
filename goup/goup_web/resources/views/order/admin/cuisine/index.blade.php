...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('store.admin.cuisine.title') }}</span>
            <h3 class="page-title">{{ __('store.admin.cuisine.title') }} {{ __('List') }}</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left">{{ __('store.admin.cuisine.title') }}</h6>

                    <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('store.admin.cuisine.add') }}</a>
                ...
                <thead>
                    <tr>
                            ...
                            <th data-value="store_type_id">{{ __('store.admin.cuisine.type') }}</th>
                            <th data-value="name">{{ __('store.admin.cuisine.name') }}</th>
...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/cuisines/create') }}", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/cuisines/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "serverSide": true,
        "ajax": {
            "url": getBaseUrl() + "/admin/store/cuisines",
            "type": "GET",
        ...
        "columns": [
            ...
            { "data": "store_type_id" ,render: function (data, type, row) {
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/admin/store/cuisines/"+id+'/updateStatus?status='+value;
