...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            ...
            <h3 class="page-title">{{ __('store.admin.item.title') }} {{ __('List') }}</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left">{{ __('store.admin.item.title') }}</h6>

                    <a href="javascript:;" data-value="{{$id}}"  class="btn btn-primary pull-right add"><i class="fa fa-plus"></i> {{ __('store.admin.item.add') }}</a>
...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/shopsitems/') }}/"+$(this).data('value')+"/create", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/shopsitems/') }}/"+$(this).data('id')+"/"+{{$id}}+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "serverSide": true,
        "ajax": {
            "url": getBaseUrl() + "/admin/store/itemsindex/"+{{$id}},
            "type": "GET",
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/admin/store/items/"+id+'/updateStatus?status='+value;
