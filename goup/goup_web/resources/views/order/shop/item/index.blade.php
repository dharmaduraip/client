...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('store.admin.item.title') }}</span>
            <h3 class="page-title">{{ __('store.admin.item.title') }} List</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left">{{ __('store.admin.item.list') }}</h6>
                    @if(Helper::getDemomode() != 1)
                    <a href="javascript:;" data-value="{{$id}}"  class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('store.admin.item.add') }}</a>
                    @endif
                </div>
                ...
                <thead>
                    <tr>
                            ...
                            <th data-value="item_name">{{ __('store.admin.item.name') }}</th>
                            <th data-value="item_description">{{ __('store.admin.item.description') }}</th>
...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('shop/items/') }}/"+$(this).data('value')+"/create", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('shop/items/') }}/"+$(this).data('id')+"/"+{{$id}}+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "serverSide": true,
        "ajax": {
            "url": getBaseUrl() + "/shop/itemsindex/"+{{$id}},
            "type": "GET",
        ...
        "columns": [
            ...
            { "data": "status", render: function (data, type, row) {
                if(data == 2){
                    ...
                    return "<label class='switch'><input type='checkbox' class='status switch-warning' checked data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>"
                   }else{
                    return "<label class='switch'><input type='checkbox' class='status'  data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>"
                   }
            }},
            ...
            var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                   '<ul class="dropdown-menu">';
                    if({{Helper::getDemomode()}} != 1){
                   button +='<li > <a class="dropdown-item status" data-id="'+data.id+'" data-value="'+data.status+'"  href="javascript:;"><i class="fa fa-plus" aria-hidden="true" title="Status">&nbsp;'+status+'</i></a></li><li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>';
                    }
                    button +='</ul> </li></div>';
                 return  button;
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/shop/items/"+id+'/updateStatus?status='+value;
