...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            ...
            <h3 class="page-title">{{ __('store.admin.category.title') }} List</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    ...
                    <a href="javascript:;" data-value="{{$id}}" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('store.admin.category.add') }}</a>
                    @endif
...
$(document).ready(function() {
   ...
   $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('shop/category/') }}/"+$(this).data('value')+"/create", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('shop/category/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "serverSide": true,
        "ajax": {
            "url": getBaseUrl() + "/shop/categoryindex/"+{{$id}},
            "type": "GET",
        ...
        "columns": [
            ...
            var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                   '<ul class="dropdown-menu">';
                    if({{Helper::getDemomode()}} != 1){
                   button +='<li > <a class="dropdown-item status" data-id="'+data.id+'" data-value="'+data.store_category_status+'"  href="javascript:;"><i class="fa fa-plus" aria-hidden="true" title="Status">&nbsp;'+status+'</i></a></li><li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>';
                    }
                    button +='</ul> </li></div>';
                 return  button;  
        ...
        var value = $(this).data('value');
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                .off()
                .on("click", function() {
                 var url = getBaseUrl() + "/shop/category/"+id+'/updateStatus?status='+value;
                  $.ajax({
                        type:"GET",
                        url: url,
                        headers: {
