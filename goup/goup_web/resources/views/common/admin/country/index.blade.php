...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('Country') }}</span>
            <h3 class="page-title">{{ __('Country') }} {{ __('List') }}</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left">{{ __('Country') }}</h6>

                    <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('Add New') }} {{ __('Country') }}</a>
...
$(document).ready(function() {
    ...
    $('body').on('click', '.edit', function(e) {
        ...
        $.get("{{ url('admin/country/') }}/"+$(this).data('id')+"/"+$(this).data('country_id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
            ...
            { "data": function (data, type, row) {
                ...
                if(data.status ==1){
                    ...
                    var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                           ...
                           button +='<li><a href="javascript:;" data-id="'+data.id+'" data-country_id="'+data.country_id+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li><li><a href="javascript:;" data-id="'+data.country_id+'" class="dropdown-item bankform"><i class="fa fa-address-card"></i>&nbsp;Bank Form</a> </li>';
                            button +='</ul> </li></div>';
                         return  button;
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/admin/companycountries/"+id+'/updateStatus?status='+value;
