...
@section('title') {{ __('Dispatcher Manager') }} @stop
...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('Dispatchers') }}</span>
            <h3 class="page-title">{{ __('Dispatcher') }} {{ __('List') }}</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0">{{ __('Dispatchers') }}</h6>
                </div>
                ...
                <div class="col-md-12">
                    <div class="note_txt">
                        ...
                        <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('Add New') }} {{ __('Dispatcher') }}</a>
...
$(document).ready(function() {
    ...
    var url = getBaseUrl() + "/admin/dispatcher";
    var keys = {id: "S.No.", name: "Dispatcher Name", email: "Dispatcher Email", mobile: "Mobile", status: "Status"};

    datatable_export(url, keys, {{Helper::getEncrypt()}});

    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/dispatchermanager/create') }}", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/dispatchermanager/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    $('body').on('click', '.delete', function() {
        var id = $(this).data('id');
        var url = getBaseUrl() + "/admin/dispatchermanager/"+id;
