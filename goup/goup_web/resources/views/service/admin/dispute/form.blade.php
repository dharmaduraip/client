...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            @if(empty($id))
                @php($action_text=__('service.admin.dispute.add'))
            @else
...
$('#namesearch').autocomplete({
	...
	select: function (event, ui) {  
		...
		$.ajax({
			url:  getBaseUrl() +"/admin/service/dispute-service-search",
			type: 'post',
			headers: {
                "Authorization": "Bearer " + getToken("admin")
            },
			...
			success:function(data, textStatus, jqXHR) {
				...
				if(data.data.length > 0) {
					...
					for(var i in result) {
						html +=`<tr><td>`+result[i].booking_id+`</td><td>`+result[i].service.service_name+`</td><td><input name="request_id" value="`+result[i].id+`" type="radio" /><input name="user_id" value="`+result[i].user_id+`" type="hidden" /><input name="provider_id" value="`+result[i].provider_id+`" type="hidden" /></td></tr>`;
					}
...
function get_disputes(dispute_type){
	$.ajax({
		url:  getBaseUrl() +"/admin/service/disputelist",
		type: 'get',
		headers: {
                "Authorization": "Bearer " + getToken("admin")
            },
