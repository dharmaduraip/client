...
@section('title') {{ __('delivery.admin.vehicletype.title') }} @stop
...
<div class="main-content-container container-fluid px-4"> 
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('delivery.admin.vehicletype.title') }}</span>
            <h3 class="page-title">{{ __('delivery.admin.vehicletype.title') }} </h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left" >{{ __('delivery.admin.vehicletype.title') }}</h6>
                    
                    <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('delivery.admin.vehicletype.add') }}</a>
                ...
                <thead>
                    <tr>
                            ...
                            <!-- <th>{{ __('delivery.admin.vehicle.type') }}</th> -->
                            <th data-value="vehicle_name">{{ __('delivery.admin.vehicle.name') }}</th>
                            <th data-value="vehicle_image">{{ __('delivery.admin.vehicle.image') }}</th>
                            <th data-value="vehicle_marker">{{ __('delivery.admin.vehicle.marker') }}</th>
                            <th data-value="vehicle_marker">{{ __('delivery.admin.vehicle.type') }}</th>
                            <!-- <th data-value="capacity">{{ __('delivery.admin.vehicle.capacity') }}</th> -->
                            <th data-value="height">{{ __('delivery.admin.vehicle.height') }}</th>
                            <th data-value="length">{{ __('delivery.admin.vehicle.length') }}</th>
                            <th data-value="weight">{{ __('delivery.admin.vehicle.weight') }}</th>
                            <th data-value="breadth">{{ __('delivery.admin.vehicle.breadth') }}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding-bottom: 35px;">
                <div class="col-md-12">                    
                        @include('delivery.admin.vehicle.priceform') 
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/deliveryvehicle/create') }}", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/deliveryvehicle/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "serverSide": true,
        "ajax": {
            "url": getBaseUrl() + "/admin/deliveryvehicle",
            "type": "GET",
        ...
        "columns": [
            ...
            { "data": "delivery_type", render: function (data, type, row) {
            ...
            { "data": function (data, type, row) {
                ...
                if({{Helper::getDemomode()}} != 1){
                    ...
                    <li><a href="javascript:;" data-toggle="modal" data-target="#service_type_2" onclick="priceCountries(`+data.id+`)" class="dropdown-item price"><i class="fa fa-money"></i> Price</a> </li>
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/admin/deliveryvehicle/"+id+'/updateStatus?status='+value;
...
function priceCountries(id) {
    var url = getBaseUrl() + "/admin/delivery/price/get/"+id;
    $.ajax({
        url: url,
        type: "GET",
        async : false,
            ...
            $.each( data.responseData,function(key,country){
                ...
                $.each( country.city,function(key1,city){
                    if(key == 0 && key1 ==0){
                      ...
                      getData(id,city.id,country.id,country.currency);
                     } else{
                         var cityActiveClass ='';
                    }
                    
                    countryCityList +=  `<a href="#" class="list-group-item cityActiveClass  `+cityActiveClass+`" data-value="`+country.currency+`" onclick="getData(`+id+`,`+city.id+`,`+country.id+`)" id="`+city.id+`"><span>`+city.city_name+` (`+city.state.state_name+`)</span></a>`;
                });
            });
...
$('body').on('click', '.addService', function(event) {
    event.preventDefault();
        $.ajax({
        url: getBaseUrl() + "/admin/delivery/comission",
        type: 'POST',
...
function getData(vehicleId,cityId,countryId,currency_value){
    ...
    $('#'+cityId).addClass(" active");
        ...
        var url = getBaseUrl() + "/admin/deliveryprice/"+vehicleId+"/"+cityId;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data.responseData.price.length !=0){
                        ...
                        $("#ride_price_id").val(ride_price_id);
                        $('#calculator').val(calculator);
                        $('#distance').val(priceData.distance);
                        $('#weight').val(priceData.weight);
                        $('#fixed').val(priceData.fixed);
                        $('#price').val(priceData.price);
                        $('#commission').val(priceData.commission);
                        $('#fleet_commission').val(priceData.fleet_commission);
                        $('#tax').val(priceData.tax);
                        $('#peak_commission').val(priceData.peak_commission);
                        
                                      
                    }else{
                        $("#ride_price_id, #distance, #weight, #distance, #fixed, #price, #commission, #fleet_commission, #tax, #peak_commission").val('');
                    }
    ...
    $('body').on('click', '.addPrice', function(event) {
        event.preventDefault();
            $.ajax({
            url: getBaseUrl() + "/admin/deliveryprice",
            type: 'POST',
...
$('body').on('click', '#calculator', function(event) {   
    ...
    function priceInputs(cal){
        ...
        if(cal=='DISTANCE'){
            ...
            $("#distance,#price").attr('value',''); 
            $("#price,#distance").prop('disabled', true);
            $("#price,#distance").prop('required', false);
            $("#weight").prop('disabled', false);
            $("#weight").prop('required', true);
            // $("#changecal").text('BP + (TKgs - BKgs*PKgm)');
            $("#changecal").text('BP + (Tw*PKgm)');
        }
        else if(cal=='DISTANCEWEIGHT'){
            $("#price,#distance,#weight").prop('disabled', false);
            $("#price,#distance,#weight").prop('required', true);
            // $("#changecal").text('BP + ((T{{__("transport.admin.vehicletype.distance")}}-BD)*P{{__("transport.admin.vehicletype.distance")}}) + (TKgs - BKgs*PKgm)');
             $("#changecal").text('BP + ((T{{__("transport.admin.vehicletype.distance")}}-BD)*P{{__("transport.admin.vehicletype.distance")}}) + (TKgs*PKgm)');
        }
        else{
            $("#minute,#weight").attr('value',''); 
            $("#price,#distance").prop('disabled', false);
            $("#price,#distance").prop('required', true);
