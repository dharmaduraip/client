...
@section('title') {{ __('delivery.admin.packagetype.title') }} @stop
...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('delivery.admin.packagetype.title') }}</span>
            <h3 class="page-title">{{ __('delivery.admin.packagetype.title') }} </h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left">{{ __('delivery.admin.packagetype.title') }}</h6>
                    
                    <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('delivery.admin.packagetype.add') }}</a>
                ...
                <thead>
                    <tr>
                            ...
                            <th data-value="delivery_name">@lang('delivery.admin.packagetype.package_name')</th>
...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/packagetype/create') }}", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/packagetype/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "serverSide": true,
        "ajax": {
            "url": getBaseUrl() + "/admin/packagetype",
            "type": "GET",
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/admin/packagetype/"+id+'/updateStatus?status='+value;
