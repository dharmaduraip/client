...
@section('title') {{ __('Overall Delivery Statements') }} @stop
...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('Overall Delivery Statements') }}</span>
            <h3 class="page-title">{{ __('Overall Delivery Statements') }}</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0">{{ __('Overall Delivery Statements') }}</h6>
                </div>
                ...
                <div class="col-lg-12 col-md-12 col-12">         		
					<form class="form-horizontal" action="{{route('admin.delivery.statement.range')}}" method="GET" enctype="multipart/form-data" role="form">
                        ...
                        <div class="row">                            
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 mt-10">
                                ...
                                <div class="col-xs-8 text-center">
                                    <a href="{{route('admin.ride.statement.range')}}" class="btn btn-primary">{{ __('Reset') }}</a>
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <div class="card dashboard_card">
                            <div class="card-header card-header-warning card-header-icon">
                                ...
                                <p class="card-category stats-small__label text-uppercase">{{ __('Total No. of Rides') }}</p>
                                <h3 class="card-title"><span id="total_rides">0</span></h3>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-4 col-sm-4">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <div class="card dashboard_card">
                            <div class="card-header card-header-warning card-header-icon">
                                ...
                                <p class="card-category stats-small__label text-uppercase">{{ __('Cancelled Rides') }}</p>
                                <h3 class="card-title"><span id="cancelled_rides">0</span></h3>
                            </div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    var url = getBaseUrl() + "/admin/delivery/requestStatementhistory";
    ...
    $('body').on('click', '.view', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/delivery-riderequestdetails/') }}/"+$(this).data('id')+"/view", function(data) {
    ...
    table = table.DataTable( {
        ...
        "ordering": false,
        "ajax": {
            "url": getBaseUrl() + "/admin/delivery/requestStatementhistory?country_id="+countryVal+"&type="+typeVal+"&from="+FromDate+"&to="+ToDate+'&fleet_id='+fleet_id+'&status='+status+'&provider_id='+provider_id+'&user_id='+user_id+'&ride_type='+ride_type,
            "type": "GET",
            'beforeSend': function (request) {
                showLoader();
            },
            ...
            dataFilter: function(data){
                ...
                json.recordsTotal = json.responseData.rides.total;
                json.recordsFiltered = json.responseData.rides.total;
                json.data = json.responseData.rides.data;
                var totalRides = json.responseData.total_rides;
                var revenueValue = json.responseData.revenue_value;
                var cancelledRides = json.responseData.cancelled_rides;
                $("#total_rides").html(totalRides);
                $("#revenue_value").html(revenueValue);
                $("#cancelled_rides").html(cancelledRides);
                hideLoader();
...
function getdata(country_id){
    ...
    if(country_id){
        $.ajax({
            url: getBaseUrl() + "/admin/getdata?country_id="+country_id+'&service_type=DELIVERY',
            type: "get",
            async: false,
            headers: {
                   Authorization: "Bearer " + getToken("admin")
            ...
            success: (data, textStatus, jqXHR) => {
               ...
               for (var i in ride_types) {
                    if(ride_type == ride_types[i].id){
                        $('select[name=ride_type]').append(`<option  selected value="` + ride_types[i].id + `">` + ride_types[i].delivery_name + `</option>`);
                    }else{
                        $('select[name=ride_type]').append(`<option value="` + ride_types[i].id + `">` + ride_types[i].delivery_name + `</option>`);
                    }
               }
            },
            error: (jqXHR, textStatus, errorThrown) => {
