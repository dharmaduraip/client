...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            @if(empty($id))
                @php($action_text=__('admin.dispute.add_delivery_dispute'))
            @else
                @php($action_text=__('admin.dispute.update_delivery_dispute'))
            @endif    
                   ...
                   <div class="form-group col-md-6">
					...
					<label for="user" class="col-xs-2 col-form-label">@lang('admin.dispute.dispute_user') / @lang('admin.dispute.dispute_provider')</label>
							<input class="form-control" type="text" value="{{ old('name') }}" name="name" id="namesearch" placeholder="Search Name" required="" aria-describedby="basic-addon2" autocomplete="off">
						 	<i class="input-group-addon fa fa-search"  id="basic-addon2"></i>
...
$(document).ready(function()
{
     ...
     if($("input[name=id]").length){
        ...
        var url = getBaseUrl() +"/admin/delivery/requestdispute"+id;
        setData( url );
     }
...
$('#namesearch').autocomplete({
    source: function(request, response) {
    	var url= getBaseUrl() +"/admin/delivery/usersearch";
    	sflag=0;
    	if($("#dispute_type").val()=='provider'){
    		sflag=1;
    		url= getBaseUrl() +"/admin/delivery/userprovider";
    	}
	    $.ajax
	    ({
	        type: "GET",
	...
	select: function (event, ui) {  
		...
		$.ajax({
			url:  getBaseUrl() +"/admin/delivery/disputeridesearch",
			type: 'post',
			headers: {
                "Authorization": "Bearer " + getToken("admin")
            },
			...
			success:function(data, textStatus, jqXHR) {
				...
				if(data.data.length > 0) {
					...
					requestList.html('');
					for(var i in result) {
						requestList.append(`<tr><td>`+result[i].booking_id+`</td><td>`+result[i].s_address+`</td><td>`+result[i].d_address+`</td><td><input name="request_id" value="`+result[i].id+`" type="radio" /><input name="user_id" value="`+result[i].user_id+`" type="hidden" /><input name="provider_id" value="`+result[i].provider_id+`" type="hidden" /></td></tr>`);
					}
				} else {
...
function get_disputes(dispute_type){
	$.ajax({
		url:  getBaseUrl() +"/admin/delivery/disputelist",
		type: 'get',
		headers: {
                "Authorization": "Bearer " + getToken("admin")
            },
