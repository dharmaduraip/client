...
console.log('time left to respond:');
                var data = parseData(response);
                if((data.responseData.requests.length > 0)) {
console.log(data.responseData.requests[0]);
console.log(data.responseData.requests[0].time_left_to_respond);
//alert('hi');
                    this.setState({
                        ...
                        requests:
                            (data.responseData.requests.length > 0)
                                ...
                                : {},
                        time:
                            (data.responseData.requests.length > 0 && data.responseData.requests[0].time_left_to_respond !== undefined)
...
console.log('this.state.requests.time_left_to_respond');
console.log(this.state.requests.time_left_to_respond);
//alert('hi');
        ...
        if( Object.keys(this.state.requests).length > 0 ) {
            if(this.state.account_status == "DOCUMENT" || this.state.account_status == "CARD" || this.state.account_status == "SERVICES" || this.state.account_status == "ONBOARDING") {
                ...
                if (status == "SEARCHING") {
                    return <RequestComponent request={this.state.requests} time={(this.state.requests.time_left_to_respond !== undefined ? this.state.requests.time_left_to_respond : 0)} />;
                    ...
                    } else if (this.state.requests.service.admin_service == "DELIVERY") {
                        window.location.replace("/provider/delivery");
                    }else{
                        window.location.replace("/provider/home");
                    }
                }
...
class RequestComponent extends React.Component {
        ...
        $.ajax({
            ...
            success: (response, textStatus, jqXHR) => {
                ...
                if(this.props.request.service.admin_service == "TRANSPORT") {
                    ...
                    window.location.replace("/provider/service");
                } else if (this.props.request.service.admin_service == "DELIVERY") {
                    window.location.replace("/provider/delivery");
    ...
    render() {
        var rows=[];
        console.log('sf',this.props.request.allow_image);
        ...
        return (            
            ...
            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
             ...
             {this.props.request.request.allow_image ?
                        <div>
                            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
                                <label htmlFor="last_name" className="font-weight-bold">Service Image : </label>
                            </div>
                            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
                                <a href={this.props.request.allow_image} target="_blank" ><img src={this.props.request.request.allow_image} className="user-img" /> </a>
                            </div>
                        </div>
                        :'' }


                {this.props.request.request.allow_description != null && (this.props.request.request.allow_description).trim() != "" ?
                <div>
                    <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
                        <label htmlFor="last_name" className="font-weight-bold">Description : </label>
                    </div>
                    <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
                        <p>{this.props.request.request.allow_description} </p>
                    </div>
                </div>
                :'' }
            ...
            {this.props.request.service.admin_service == "TRANSPORT" || this.props.request.service.admin_service == "DELIVERY" ?
            <div><h5>Pickup Location</h5><p>{this.props.request.request.s_address}</p></div> :'' }
            {this.props.request.service.admin_service == "DELIVERY" ?
            <div><h5>Dropping Point {this.props.request.request.deliveries.length && this.props.request.request.deliveries.length > 1 ? 's ' + (this.props.request.request.deliveries.length) : ''}  </h5> <p>{this.props.request.request.deliveries.map((delivery) => <span key={delivery.id}> {delivery.d_address} </span>)}</p></div> : ''}
            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
                {
                    this.props.request.request.deliveries != null ?
                        this.props.request.request.deliveries.map((value, index) => { 
                            return <div><img src={value.picture} className="user-img" /> </div>;
                        }) 
                        : ''
                }
            </div>
