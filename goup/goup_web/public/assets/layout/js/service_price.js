    ...
    table = table.DataTable( {
        ...
        "columns": [
            ...
            { "data": "service_status" ,render: function (data, type, row) {
                console.log(row);
                if(data ==1){
                    return "<label class='switch'><input type='checkbox' class='status_enable switch-warning' checked data-id='"+row.id+"' data-value='"+row.service_status+"'> <span class='slider round'></span></label>"
               }else{
                return "<label class='switch'><input type='checkbox' class='status_enable'  data-id='"+row.id+"' data-value='"+row.service_status+"'> <span class='slider round'></span></label>"
               }
            }
            },
            { "data": function (data, type, row) {
                ...
                return "<button onclick='priceCountries("+data.id+")' data-toggle='modal' data-target='#service_type_service' class='btn btn-block btn-outline-primary price'>Price</button><button data-id='"+data.id+"' class='btn btn-block btn-success edit'>Edit</button>";
            ...
            $.each( data.responseData,function(key,value){
                ...
                $.each( value.company_country_cities,function(key1,value1){
                    if(key == 0 && key1 ==0){
                        ...
                        $("#serviceId").val(id);
                        getData(id,value1.city.id,value.country.id,value.currency,value.distance_unit);
                    }else{
                        var cityActiveClass ='';
                    }
                    if (value1.city != null) {
...
function getData(serviceId,cityId,countryId,currency_value,distance_unit){
