class MainComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        $.ajax({
            url: getBaseUrl() + "/provider/check/delivery/request",
            type: "get",
            async: false,
            processData: false,
            contentType: false,
            ...
            success: (response, textStatus, jqXHR) => {
                ...
                if (data.responseData.request.id) {
                    ...
                    socket.emit('send_location', { room: `room_${window.room}_R${id}_DELIVERY`, latitude: '13.07322590', longitude: '80.26082090' });
                }
...
class StartedComponent extends React.Component {
    ...
    handleUpdateStatus = (id, status) => {
        ...
        let otpState = getSiteSettings().delivery.otp;
        ...
        if (otpState == 1) {
            var data = { _method: "PATCH", id, status, otp, d_latitude, d_longitude, d_address };
        } else {
            var data = { _method: "PATCH", id, status, d_latitude, d_longitude, d_address };
        }

        $.ajax({
            url: getBaseUrl() + "/provider/update/delivery/request",
            type: "post",
            data: data,
            headers: {
                Authorization: "Bearer " + getToken("provider")
            ...
            success: (data, textStatus, jqXHR) => {
                ...
                let currentDelivery = data;

                return <InvoiceComponent request={currentDelivery} provider={this.props.provider} />;
            },
            error: (jqXHR, textStatus, errorThrown) => {
                hideLoader();
    ...
    handleOtp = id => {
        ...
        if (otp != "") {
            this.handleUpdateStatus(id, 'PICKEDUP');
        } else {
    ...
    render() {
        ...
        for (var i in this.props.request.delivery) {
            image += "&markers=icon:" + window.url + "/assets/layout/images/common/marker-end.png%7C" + this.props.request.delivery.d_latitude + "," + this.props.request.delivery.d_longitude;
        }

        image += "&path=color:0x191919|weight:3|enc:" + this.props.request.route_key + "&key=" + getSiteSettings().site.browser_key;

        var delivery = this.props.request.delivery;
        ...
        return (
            <div className="row">
                <div className="service-card col-xs-12 col-sm-12 col-md-12 col-lg-6 available m-0" >

                    {/*<div className="dropoff-container">
                { this.props.request.delivery.map((d) => <div key={d.id} className={d.id === delivery.id ? 'dropoff-box active' : 'dropoff-box'}>{(d.d_address).split(',')[0]} - Dropoff</div> )}

            </div>*/}


                    {delivery && <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0">
                        <div className="form-register">
                            <div className="steps clearfix">
                                <ul role="tablist">
                                    <li id="arrived_tab" role="tab" aria-disabled="false" className={(this.state.status == "ARRIVED" || this.state.status == "PICKEDUP" || this.state.status == "STARTED" || this.state.status == "DROPPED" || this.state.status == "COMPLETED") ? "current" : ""} aria-selected="true">
                                        <a id="form-total-t-0" aria-controls="form-total-p-0">
                                            <span className="current-info audible"> </span>
                                            <div className="title">
                                                <span className="step-icon"><img src="/assets/layout/images/delivery/svg/map.svg" /></span>
                                                <span className="step-text">Arrived</span>
                                            </div>
                                        </a>
                                    </li>
                                    <li id="picked_tab" role="tab" aria-disabled="false" className={(delivery.status == "STARTED" || delivery.status == "DROPPED" || delivery.status == "COMPLETED") ? "current" : ""} aria-selected="false">
                                        <a id="form-total-t-1" aria-controls="form-total-p-1">
                                            <div className="title">
                                                <span className="step-icon"><img src="/assets/layout/images/delivery/svg/taxi.svg" /></span>
                                                <span className="step-text">Started</span>
                                            </div>
                                        </a>
                                    </li>
                                    <li id="reached_tab" role="tab" aria-disabled="false" className={(delivery.status == "DROPPED" || delivery.status == "COMPLETED") ? "current" : ""} aria-selected="false">
                                        <a id="form-total-t-2" aria-controls="form-total-p-2">
                                            <div className="title">
                                                <span className="step-icon"><img src="/assets/layout/images/delivery/svg/destination.svg" /></span>
                                                <span className="step-text">Reached Location</span>
                                            </div>
                                        </a>
                                    </li>
                                </ul>
                        ...
                        <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0">
                            ...
                            <div className="user-right">
                                ...
                                <div className="rating-outer">
                                    ...
                                    <input type="hidden" className="rating" delivery="1" disabled="disabled" />
                                </div>
                            </div>
                        </div>
                        <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0">
                            ...
                            <h4>Booking ID</h4>
                            <h5>{this.props.request.booking_id}</h5>
                            <h4>Receiver Details</h4>
                            <h5>{delivery.name}</h5>
                            <p> <a href={'tel:' + delivery.mobile}>{delivery.mobile}</a>  </p>
                        ...
                        {this.props.request.payment_by == "RECEIVER" && delivery.status == "DROPPED" ?
                            <div className="col-xs-12 col-sm-12 col-md-10 col-lg-11 p-0">
                                
                                {this.props.request.delivery.payment ? (
                                                                    <span>
                                                                        <h4>Invoice Amount</h4>
                                                                        <h4>
                                                                            {this.props.request.currency}{(this.props.request.delivery.payment.payable).toFixed(2)}
                                                                        </h4>
                                                                    </span>
                                                                ) : ("")}

                            </div> : <div className="col-xs-12 col-sm-12 col-md-10 col-lg-11 p-0">
                                <h5>Pickup Location</h5>
                                <p>{this.props.request.s_address}</p>
                                {(this.props.request.is_drop_location == 1) ?
                                    <div>
                                        <h5>Drop Location</h5>
                                        <p>{delivery.d_address}</p>
                                    </div>
                                    : ''}
                          ...
                          {this.props.request.payment_by == "SENDER" && this.props.request.status == "ARRIVED" ?
                            <div className="col-xs-12 col-sm-12 col-md-10 col-lg-11 p-0">
                                {this.props.request.delivery.payment ? (
                                    <span>
                                    <h4>Invoice Amount</h4>
                        ...
                        <div className="actions col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            {delivery.status == "PROCESSING" && this.props.request.status == "ARRIVED" && this.props.request.payment_by == "SENDER" ?
                                this.props.request.payment_mode == "CASH" && this.props.request.paid != 1 || this.props.request.payment_by == "RECEIVER" && this.props.request.paid != 1 ?
                                    <a id="pay" className="btn btn-primary btn-md mr-2" onClick={() => { this.handleUpdateStatus(delivery.id, 'PAYMENT') }}>Paid <i className="fa fa-check" aria-hidden="true"></i></a> : 
                                    this.props.request.status == "ARRIVED" ?  <a className="btn btn-primary btn-md mr-2" >Waiting for Payment</a> : 
                                        <a id="arrived" className="btn btn-primary btn-md mr-2" onClick={() => { this.handleUpdateStatus(delivery.id, 'ARRIVED') }}>Arrived <i className="fa fa-check" aria-hidden="true"></i></a>
                                : ''}
                            {delivery.status == "PROCESSING" && this.props.request.status == "ARRIVED" && this.props.request.payment_by == "RECEIVER" ?
                                        <a id="arrived" className="btn btn-primary btn-md mr-2" onClick={() => { this.handleUpdateStatus(delivery.id, 'PICKEDUP') }}>Picked Up <i className="fa fa-check" aria-hidden="true"></i></a>
                                : ''}
                            {delivery.status == "PROCESSING" && this.props.request.status == "STARTED" ?
                               <a id="arrived" className="btn btn-primary btn-md mr-2" onClick={() => { this.handleUpdateStatus(delivery.id, 'ARRIVED') }}>Arrived <i className="fa fa-check" aria-hidden="true"></i></a>
                                : ''}
                            {/*(delivery.status == "STARTED" && getSiteSettings().delivery.otp == "1" && this.props.request.created_type != "ADMIN") ? <a id="arrived" className="btn btn-primary btn-md mr-2" data-toggle="modal" onClick={this.handleOtpModal} >Pick <i className="fa fa-check" aria-hidden="true"></i></a> : "" */}
                            {(delivery.status == "STARTED" && (getSiteSettings().delivery.otp == "0" || this.props.request.created_type != "ADMIN")) ? <a id="arrived" className="btn btn-primary btn-md mr-2" onClick={() => { this.handleUpdateStatus(delivery.id, 'DROPPED') }} >Reached Delivery Location <i className="fa fa-check" aria-hidden="true"></i></a> : ""}
                            ...
                            {delivery.status == "DROPPED" ?
                                <div>
                                    ...
                                    <a id="dropped" className="btn btn-primary btn-md mr-2" onClick={() => { this.handleUpdateStatus(delivery.id, 'COMPLETED') }}> { this.props.request.payment_by == "SENDER" ? "Delivered" : "Paid & Delivered"  } </a>
                                </div>
                                : ""}
                        </div>
                        {/* <!-- Popup Modal --> */}
                                ...
                                <div className="modal-content">
                                    ...
                                    <div className="modal-body">
                                        <div className="dis-center">
                                            <img src="/assets/layout/images/delivery/svg/otp.svg" className="w-30" />
                                        </div>
                                    ...
                                    <div className="modal-footer">
                                        <a id="startTrip" className="btn btn-primary btn-block" onClick={() => { this.handleOtp(delivery.id) }}>Start Trip</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {this.props.request.is_drop_location == 1 ?
                    <div className="col-sm-12 col-md-12 col-lg-6 map-section">
                        <img style={{ boxShadow: "2px 2px 10px #ccc", width: "100%" }} src={image} />
                    </div> : ''}
...
class InvoiceComponent extends React.Component {
    ...
    handlePayment = e => {
        ...
        $.ajax({
            url: getBaseUrl() + "/provider/delivery/payment",
            type: "post",
            data: {
                id
            },
    ...
    handleRating = e => {
        ...
        $.ajax({
            url: getBaseUrl() + "/provider/rate/delivery",
            type: "post",
            data: {
                    ...
                    <div className="col-md-12 col-sm-12 col-lg-12 p-0">
                        ...
                        <form id="ride_creation">
                            <dl className="dl-horizontal left-right mt-5">
                                ...
                                <hr />
                                <dt>Total</dt>
                                <dd id="total_amount">{this.props.request.currency}{this.props.request.payable_amount}</dd>
                                <hr />
                            </dl>
...
class CancelComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        $.ajax({
            url: getBaseUrl() + "/provider/reasons?type=DELIVERY",
            type: "get",
            data: {},
            headers: {
                Authorization: "Bearer " + getToken("provider")
    ...
    handleCancel = e => {
        ...
        if (this.state.reason != '') {
            $.ajax({
                url: getBaseUrl() + "/provider/cancel/delivery/request",
                type: "post",
                data: {
                    id: id,
                    reason: reason
