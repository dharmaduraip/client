...
@section('main')
    ...
    <section class="section benefits bgcolor--gray-10 mt-5 pt-5">
        ...
        <form action="{{ route('stripe.post') }}" method="POST" class="d-none">
			  @csrf
				<button type="submit"   class="stripe-button-el" id="stripe_script">Pay</button>
			</form>
...
@section('script')
    @if (config('zakirsoft.midtrans_active') &&
        ...
        @if (config('zakirsoft.midtrans_live_mode'))
            <script src="https://app.midtrans.com/snap/snap.js" data-client-key="{{ config('zakirsoft.midtrans_client_key') }}">
            </script>
        @else
