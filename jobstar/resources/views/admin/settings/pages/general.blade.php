    ...
    <div class="row">
        <div class="col-xl-6 col-lg-12">
            ...
            <div class="card">
                ...
                <form class="form-horizontal" action="{{ route('settings.analytics.update') }}" method="POST" id="analytics_form">
                    @method('PUT')
                    @csrf
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-7">
                                <input value="{{ old('analytics_id') ?? env("GOOGLE_ANALYTICS_ID") }}" name="analytics_id"
                                    type="text" class="form-control @error('analytics_id') is-invalid @enderror"
                                    autocomplete="off">
                                @error('analytics_id')
                                    <span class="invalid-feedback" role="alert"><span>{{ $message }}</span></span>
                                @enderror
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-7">
                                <input {{ old('is_analytics_active') ?? env("GOOGLE_ANALYTICS_STATUS") ? 'checked' : '' }} type="checkbox" name="is_analytics_active"
                                    data-bootstrap-switch value="1" data-on-text="{{ __('on') }}"
                                    data-off-color="default" data-on-color="success"
                                    data-off-text="{{ __('off') }}">
                            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between">
                        <h3 class="card-title line-height-36">
                            {{ __('google_job_indexing_configuration') }}
                            (<small><a href="https://developers.google.com/search/apis/indexing-api/v3/prereqs"
                                    target="_blank">{{ __('prerequisites') }}</a></small>)
                        </h3>
                    </div>
                </div>
                <form class="form-horizontal" action="{{ route('settings.indexing.update') }}" method="POST" id="job_indexing_key_form">
                    @method('PUT')
                    @csrf
                    <div class="card-body">
                        <div class="form-group row">
                            <x-forms.label name="Google Indexing Access Token" class="col-sm-5" />
                            <div class="col-sm-7">
                                <input value="{{ old('job_indexing_access_token') ?? env("GOOGLE_JOB_INDEXING_ACCESS_TOKEN") }}" name="job_indexing_access_token"
                                    type="text" class="form-control @error('job_indexing_access_token') is-invalid @enderror"
                                    autocomplete="off">
                                @error('job_indexing_access_token')
                                    <span class="invalid-feedback" role="alert"><span>{{ $message }}</span></span>
                                @enderror
                                <small class="text-danger">Note: Google Analytics must be properly configured to use Google Indexing</small>
                            </div>
                        </div>
                        @if (userCan('setting.update'))
                            <div class="form-group row">
                    ...
                    <form action="{{ route('settings.system.update') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 mt-2">
                                ...
                                <div class="form-group">
                                    <x-forms.label class="mt-2" name="frontend_currency_switcher"
                                        :required="true" />
                                    <div>
                                        <input type="hidden" name="currency_switcher" value="0" />
                                        <input type="checkbox" id="currency_switcher"
                                            {{ $setting->currency_switcher ? 'checked' : '' }}
                                            name="currency_switcher" data-on-color="success" data-bootstrap-switch
                                            data-on-text="{{ __('on') }}" data-off-color="default"
                                            data-off-text="{{ __('off') }}" data-size="small" value="1">
                    ...
                    <div id="text-card" class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                ...
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input onclick="changeMapType()" type="radio" id="mapp-box" name="map_type"
                                        class="custom-control-input" value="map-box"
                        ...
                        <div id="mapbox_key"
                            class="{{ $setting->default_map == 'map-box' ? '' : 'd-none' }} form-group row">
