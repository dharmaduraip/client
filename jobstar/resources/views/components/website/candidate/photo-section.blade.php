<x-forms.label name="profile_picture"
        class="pointer body-font-4 d-block text-gray-900 rt-mb-8" />
    <div id="photo-uploadMode" class="{{ $candidate->photo ? 'd-none' : '' }}">
        <div class="profile-image-upload-wrap">
            <input name="image" class="profile-file-upload-input dropify"
                type='file' onchange="readURL(this);" accept="image/*" data-allowed-file-extensions="jpg png svg jpeg"/>
                @error('image')
    ...
    <div id="photo-oldMode" class="{{ $candidate->photo ? '' : 'd-none' }}">
        <div class="profile-file-upload-content2">
            <img class="profile-file-upload-image" src="{{ asset($candidate->photo) }}"
                alt="your image" />
