...
class WebsiteController extends Controller
{
    ...
    public function employees(Request $request)
    {
        ...
        if ($request->has('lat') && $request->has('long') && $request->lat != null && $request->long != null) {
            $ids = $this->company_location_filter($request->lat, $request->long, $request->location);
    ...
    public function toggleApplyJob(Request $request)
    {
        ...
        DB::table('applied_jobs')->insert([
            ...
            'application_group_id' => $job->company->applicationGroups->where('is_deleteable', false)->first()->id ?? 2,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    public function company_location_filter($latitude, $longitude, $location)
    {
        // $distance = 50;

        $string = $location;
        $locate = \Str::before($string, ' ');
        ...
        $data = Company::select('id')/*->selectRaw("$haversine AS distance")
            ->having("distance", "<=", $distance)*/->where('address', 'like', '%'.$locate.'%')->orWhere('region', 'like', '%'.$locate.'%')->get();
