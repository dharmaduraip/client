...
class Job extends Model
{
    ...
    protected $appends = ['days_remaining', 'deadline_active', 'can_apply', 'full_address','salary','days_from_post'];
	...
	public function getDaysFromPostAttribute()
    {
    ...
    {
        return getFormattedNumber($this->min_salary)." - ".getFormattedNumber($this->max_salary)." ".currentCurrencyCode();
    }
    ...
    public function getCountryNewAttribute()
    {
        return \DB::table('countries')->where('id', $this->attributes['country'])->first();
    }

    public function getRegionNewAttribute()
    {
        return \DB::table('states')->where('id', $this->attributes['region'])->first();
    }

    public function getSalaryTypeAttribute()
    {
        return \DB::table('salary_types')->where('id', $this->attributes['salary_type_id'])->first();
    }

    public function getJobCategoryAttribute()
    {
        return \DB::table('job_category_translations')->where('locale', 'en')->where('job_category_id', $this->attributes['category_id'])->first();
    }

    public function getCompanyDetailAttribute()
    {
        return Company::where('id', $this->company_id)->select('id', 'user_id', 'logo')->first();
    }
}
