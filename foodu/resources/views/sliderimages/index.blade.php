...
<div class="m-sm-4 m-3 box-border">
	...
	<div class="toolbar-nav" >   
		<div class="row">
			<div class="col-md-8 button-chng"> 	
				@if($access['is_add'] ==1)
				<a href="{{ url('sliderimages/create?return='.$return) }}" class="btn  btn-sm"  
	...
	<div class="p-3">
		<div class="table-container for-icon m-0">
			...
			{!! Form::open(array('url'=>'sliderimages?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
			...
			<table class="table  table-hover " id="{{ $pageModule }}Table">
				...
				<tbody>        						
					...
					<tr>
						...
						<td>					 
							@if($field =='image')
							{!! SiteHelpers::showUploadedFile($row->{$field['field']},'/uploads/slider_img/') !!}
						...
						<td>
							...
							<a href="{{url('sliderimages/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{!! Lang::get('core.btn_view') !!}"><i class="fa  fa-search "></i></a>
							@endif
							@if($access['is_edit'] == 1)
							<a  href="{{ url('sliderimages/'.$row->id.'/edit?return='.$return) }}" class="tips btn btn-xs btn-success" title="{!! Lang::get('core.btn_edit') !!}"><i class="fa fa-edit "></i></a>
							@endif
						</td>														
