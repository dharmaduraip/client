...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'deliveryboywallet?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
						...
						<fieldset>
							...
							<div class="form-group">
								...
								<div class="col-sm-12 text-center">	
									...
									<button type="button" onclick="location.href='{{ URL::to('deliveryboywallet?search=del_boy_id:equal:'.$u_id) }}' " class="btn btn-success btn-sm "><i class="fa fa-arrow-circle-left "></i> {!! Lang::get('core.sb_cancel') !!} </button>
								</div>
							</div>
						</fieldset>
					</div>
  ...
  <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("deliveryboywallet/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
