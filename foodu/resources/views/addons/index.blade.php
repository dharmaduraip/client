...
<div class="toolbar-nav" >   
	<div class="row">
		<div class="col-md-8 my-1 button-chng"> 	
			@if($access['is_add'] ==1)
			<a href="{{ url('addons/create?return='.$return) }}" class="btn  btn-sm"  
			...
			{{-- <a href="{{ URL::to( 'addons/search') }}" class="btn btn-sm btn-white" onclick="AbserveModal(this.href,'Advance Search'); return false;" ><i class="fa fa-search"></i> {!! trans('core.btn_search') !!}</a>	 --}}  
		</div>
	</div>	
</div>		
<div class="table-container for-icon m-0">
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'addons/store'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
		            ...
		            @foreach ($rowData as $row)
		                <tr>
						 ...
						 <td>
						 	...
						 	<a href="{{url('addons/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{!! Lang::get('core.btn_view') !!}"><i class="fa  fa-search "></i></a>
						 	@endif
						 	@if($access['is_edit'] == 1)
						 	<a  href="{{ url('addons/'.$row->id.'/edit?return='.$return) }}" class="tips btn btn-xs btn-success" title="{!! Lang::get('core.btn_edit') !!}"><i class="fa fa-edit "></i></a>
						 	@endif
						 </td>		 
		                </tr>
