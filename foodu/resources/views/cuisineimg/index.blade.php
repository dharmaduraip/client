	...
	<div class="toolbar-nav" >   
		<div class="row">
			<div class="col-md-8 button-chng my-1"> 	
				@if($access['is_add'] ==1)
				<a href="{{ url('cuisineimg/create?return='.$return) }}" class="btn  btn-sm"  
				...
				<div class="dropdown-menu dropdown-menu-right" style="">
					...
					@endif
					<a href="{!! url('cuisineimg/CuisineimgExport/'.$dn) !!}" class="dropdown-item"><i class="fas fa-file-{!! $dwnicon[$dn] !!}"></i> {!! $dv !!}</a>
					@php $i++; @endphp
					@endforeach
				</div>
	...
	<div class="p-3">		
		<div class="table-container for-icon m-0">
					...
					<!-- Table Grid -->
		 			...
		 			{!! Form::open(array('url'=>'cuisineimg/store'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
				        ...
				        <tbody>        						
				            @foreach ($rowData as $row)
								...
								<td>
									...
									<a href="{{url('cuisineimg/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{!! Lang::get('core.btn_view') !!}"><i class="fa  fa-search "></i></a>
									@endif
									@if($access['is_edit'] == 1)
									<a  href="{{ url('cuisineimg/'.$row->id.'/edit?return='.$return) }}" class="tips btn btn-xs btn-success" title="{!! Lang::get('core.btn_edit') !!}"><i class="fa fa-edit "></i></a>
