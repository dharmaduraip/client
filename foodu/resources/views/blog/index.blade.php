	...
	<div class="toolbar-nav" >   
		<div class="row">
			<div class="col-md-6 button-chng my-1 "> 	
				@if($access['is_add'] ==1)
				<a href="{{ url('blog/create?return='.$return) }}" class="btn  btn-sm"  
				...
				<button type="button" class="btn bg-transparent text-success border-success dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="icon-download"></i> Download</button>
					...
					@endif
					<a href="{!! url('blog/BlogExports/'.$dn) !!}" class="dropdown-item"><i class="fas fa-file-{!! $dwnicon[$dn] !!}"></i> {!! $dv !!}</a>
					@php $i++; @endphp
					@endforeach
				</div>
...
<div class="table-container for-icon m-0">
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'blog/store'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
		            ...
		            @foreach ($rowData as $row)
		                <tr>
						 ...
						 <td>
						 	...
						 	<a href="{{url('blog/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{!! Lang::get('core.btn_view') !!}"><i class="fa  fa-search "></i></a>
						 	@endif
						 	@if($access['is_edit'] == 1)
						 	<a  href="{{ url('blog/'.$row->id.'/edit?return='.$return)  }}" class="tips btn btn-xs btn-success" title="{!! Lang::get('core.btn_edit') !!}"><i class="fa fa-edit "></i></a>
...
<script>
	$(document).ready(function(){
		...
		$('.do-quick-search').click(function(){
			$('#AbserveTable').attr('action','{{ URL::to("blog/multisearch")}}');
			$('#AbserveTable').submit();
		});
