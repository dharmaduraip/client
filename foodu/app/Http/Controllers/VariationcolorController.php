...
use App\Exports\VariationcolorExport;



class VariationcolorController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'variationcolor';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Variationcolor();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'variationcolor',
	...
	public static function display(  )
	{
		...
		$model  = new Variationcolor();
		$info = $model::makeInfo('variationcolor');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('variationcolor.public.view',$data);			
			}			
		} 
		else {
			...
			return view('variationcolor.public.index',$data);	
		}
	...
	public function variationexport(Request $request,$slug) 
        {
        $request->all();
        $exporter = app()->makeWith(VariationcolorExport::class, compact('request'));  
        return $exporter->download('VariationcolorExport_'.date('Y-m-d').'.'.$slug);
        }
}  
