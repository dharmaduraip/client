...
abstract class Controller extends BaseController {
	...
	public function blastnotification($app_api,$tokens,$message,$app_name,$device_name,$subject_title) {
		$mobile_token = $tokens;
		$device 	  = $device_name;
		$tone=1;
		$imageName='';
		
		self::FCM($mobile_token,$device,$subject_title,$message,$tone,$imageName);
	}

	public function sendpushnotification($user_id,$type,$message,$click_action='',$sub='',$tone='',$imageName='')
	{	 
		$tableName	= $type == 'delivery' ? 'abserve_deliveryboys' : 'tb_users';
		$userapi	= User::select('mobile_token','device')->where('id',$user_id)->first();
		if ( !empty($userapi) && $userapi->mobile_token != '') {
			$mobile_token	= $userapi->mobile_token;
			$device			= $userapi->device;
			self::FCM($mobile_token,$device,$sub='',$message,$tone="",$imageName="");
		}
	}

	public function FCM($mobile_token,$device,$sub='',$message,$tone="",$imageName="",$click_action='')
	{
		$appapi_details = \DB::table('abserve_app_apis')->select('*')->where('id','1')->first();
		$app_api = $appapi_details->api;
		$app_name = $appapi_details->app_name;
		// if(!define( 'API_ACCESS_KEY', $app_api )){
		// define( 'API_ACCESS_KEY', $app_api );
		// }

		if (!defined('API_ACCESS_KEY')) define('API_ACCESS_KEY', $app_api);


		if($sub!=''){
			$subject=$sub;
			$registrationIds = is_array($mobile_token) ? $mobile_token : [$mobile_token];
		}else{
		...
		$msg = array(
			...
			'sound'		=> $sound,
			'icon'		=> 'ic_launcher',
			'android_channel_id'=> time(),
			'largeIcon'	=> 'large_icon',
			'smallIcon'	=> 'small_icon',
			'click_action' => $click_action
		);
		$path = 'uploads/notification/'.$imageName;
		if($imageName != '' && file_exists(base_path($path))){
			$msg['image'] = url($path);
		}
		...
		if($sub!=''){
			...
			if($device=='ios'){
				...
				if($imageName != '' && file_exists(base_path($path))){
					$fields['urlImageString'] = url($path);
				}
			}else{
				$fields = array(
					'registration_ids'     => $registrationIds,
	...
	function buildSearch( $map = false)
	{
		...
		if((\Auth::user()->group_id == 1 || \Auth::user()->group_id == 2) && $this->info['table'] == 'abserve_hotel_items'){
			$this->model = new Restaurant();
			$this->info = $this->model->makeInfo('restaurant');	
		}
		...
		if($_GET['search'] !='')
		{
			$type = array_filter(explode("|",$_GET['search'] ));
			if(count($type) >= 1)
			{
				foreach($type as $t)
				{
					$keys = explode(":",$t);
					if( \Request::segment(1)	== 'weeklypaymentforhost' && $keys[0]	== 'date') {
					$data	= \DB::table('abserve_order_details')->select('partner_id')->whereBetween(\DB::raw('DATE(date)'), [$keys[2], $keys[3]])->whereBetween(\DB::raw('DATE(date)'), [$keys[2], $keys[3]])->where('status','4')->pluck('partner_id');
					$ids	= implode(',', $data->toArray());
					if (isset($data) && count($data)>0) {			
						$param .= " AND id"." " ."IN"."(".$ids.")";
					} else {
						$param .= " AND id"." " ."IN"."(NULL)";		
					}	
					continue;
					}
					if( \Request::segment(1)	== 'weeklypaymentforhost') {
					$data	= \DB::table('abserve_order_details')->select('partner_id')->where('status','4')->where('res_id',$keys[2])->pluck('partner_id');
					$ids	= implode(',', $data->toArray());
					if (isset($data) && count($data)>0) {			
						$param .= " AND id"." " ."IN"."(".$ids.")";
					} else {
					...
					if( \Request::segment(1)	== 'weeklypaymentfordeliveryboy' && $keys[0]	== 'date') {
					// echo "<pre>";print_r(date('Y-m-d H:i:s',strtotime($keys[2])));echo "<br>";print_r(date('Y-m-d H:i:s',strtotime($keys[3])));exit();
					// \DB::enableQueryLog();
					$data  = \DB::table('abserve_rider_location_log')->select('boy_id')->whereIn('order_status',['boyPicked','boyArrived','4','2'])->whereBetween('created_at',[date('Y-m-d H:i:s',strtotime($keys[2])),date('Y-m-d H:i:s',strtotime($keys[3]))])->pluck('boy_id');
					// $boy_id[] = '';
					/*foreach (json_decode($data) as $key => $value) {
						$distance = \AbserveHelpers::getDel_boy_payable_weekly_amt($value,$keys[2],$keys[3]);
						if ($distance > 0) {
							$boy_id[] = $value;
						}
					}*/
					$ids	= implode(',', array_filter(json_decode($data)));
					if (isset($data) && count($data)>0) {			
						$param .= " AND id"." " ."IN"."(".$ids.")";
					} else {
						$param .= " AND id"." " ."IN"."(NULL)";		
					...
					if(in_array($keys[0],array_keys($arr))):
						...
						} else if($arr[$keys[0]]['type'] == "text_datetime" && $keys[1] == "between"){
							$param .= " AND ".$arr[$keys[0]]['alias'].".".$keys[0]." ".self::searchOperation($keys[1])." '".date('Y-m-d H:i:s',strtotime($keys[2].' 12:00 AM'))."' AND '".date('Y-m-d H:i:s',strtotime($keys[3].' 11:59 PM'))."'";
						} else {
	...
	function validateForm( $forms = array() )
	{
		...
		{
			if(($form['required'] == '' || $form['required'] !='0') && ( $form['required'] != null ) )
			{
					...
					{	
						...
						if($f['option']['upload_type'] =='file')
						{
							...
							if(isset($f['option']['image_multiple']) && $f['option']['image_multiple'] ==1)
							{
								...
								if(!is_null($request->file($field)))
								{
									if($f['alias'] == "abserve_deliveryboys"){
										$f['option']['path_to_upload'] = '/uploads/deliveryboy';
									}
									if($f['alias'] == "tb_users"){
										$f['option']['path_to_upload'] = '/uploads/users';
									}
									if($f['alias'] == 'abserve_banner'){
										$f['option']['path_to_upload'] = '/uploads/banner/';
									}
									if($f['alias'] == 'abserve_slider_images'){
										$f['option']['path_to_upload'] = '/uploads/slider';
									}
									if($f['alias'] == 'abserve_promocode'){
										$f['option']['path_to_upload'] = '/uploads/slider';
									}
									if($f['alias'] == 'abserve_food_cuisines'){
										$f['option']['path_to_upload'] = '/uploads/cuisine/';
									}
									if($f['alias'] == 'location'){
										$f['option']['path_to_upload'] = '/uploads/location_ad/';
									}
									if($f['alias'] == 'abserve_blog'){
										$f['option']['path_to_upload'] = '/uploads/blogs/';
									}
									if($f['alias'] == 'abserve_food_categories'){
										$f['option']['path_to_upload'] = '/uploads/categories/';
									}
									$file = $request->file($field); 
								 	$destinationPath = '.'. $f['option']['path_to_upload'];  
									$filename = $file->getClientOriginalName();
	...
	function validateDatas($data,$rules,$custom_message=[],$niceNames = [])
	{
		...
		if($validator->passes()){
			...
			$message['message'] = \AbserveHelpers::getValidationErrorMsg($validator);
