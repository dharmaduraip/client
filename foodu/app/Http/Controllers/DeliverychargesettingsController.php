...
class DeliverychargesettingsController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'deliverychargesettings';
	static $per_page	= '20';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Deliverychargesettings();	

		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'deliverychargesettings',
...
// Render into template
	...
	function create( Request $request , $id =0 ) 
	{
		...
		if(\Auth::user()->group_id == 5 || \Auth::user()->group_id == 2){
			$this->data['location'] = Location::where('id',\Auth::user()->location)->get();
		} else{
			$this->data['location'] = Location::get();
		}
...
// redirect
	...
	public static function display(  )
	{
		...
		$model  = new Deliverychargesettings();
		$info = $model::makeInfo('deliverychargesettings');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('deliverychargesettings.public.view',$data);			
			}			
		} 
		else {
			...
			return view('deliverychargesettings.public.index',$data);	
		}
	...
	public function deliveryexport(Request $request,$slug) 
	{       	
		$request->all();
		$exporter = app()->makeWith(DeliverychargeExport::class, compact('request'));  
		return $exporter->download('DeliverychargeExport_'.date('Y-m-d').'.'.$slug);
	}
}
