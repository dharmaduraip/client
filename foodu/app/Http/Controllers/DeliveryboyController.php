...
class DeliveryboyController extends Controller {
	...
	public function deliveryboyexport(Request $request,$slug)
    {
        $request->all();
        $exporter = app()->makeWith(DeliveryboyExport::class, compact('request'));  
        return $exporter->download('DeliveryboyExport_'.date('Y-m-d').'.'.$slug);
    }
	...
	function store( Request $request  )
	{
		...
		switch ($task)
		{
			...
			if($id =='')
			{
				...
				$rules['phone_number'] 			= 'required|numeric|digits:10|unique:tb_users';
			}else { 
	...
	public static function display(  )
	{
		...
		$info = $model::makeInfo('deliveryboy');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('deliveryboy.public.view',$data);			
			}			
		} 
		else {
			...
			return view('deliveryboy.public.index',$data);	
		}
	}
	...
	public function getBlockdelboy(Request $request)
	{
	...
	public function getDeletedelboy(Request $request)
	{
		$explodeid = explode(',', rtrim($request->getid,','));
		foreach ($explodeid as $key => $value) {
			\DB::table('tb_users')->where('id',$value)->delete();
			\DB::table('tb_delboy_accounts')->where('delboy_id',$value)->delete();
			...
			if(SOCKET_ACTION == 'true'){
				...
				$client->emit('rider delete', $aData);
			}	
		}
		echo '1';
	}
