...
use App\Exports\CuisineimgExport;
...
class CuisineimgController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'cuisineimg';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Cuisineimg();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'cuisineimg',
	...
	function store( Request $request  )
	{
		...
		switch ($task)
		{
			default:
				...
				if($id == ''){
                    $rules['image']  = 'required|mimes:jpg,jpeg,png,gif,bmp';
				}
	...
	public static function display(  )
	{
		...
		$model  = new Cuisineimg();
		$info = $model::makeInfo('cuisineimg');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('cuisineimg.public.view',$data);			
			}			
		} 
		else {
			...
			return view('cuisineimg.public.index',$data);	
		}
	...
	public function cuisineexport(Request $request,$slug) 
        ...
        $request->all();
        $exporter = app()->makeWith(CuisineimgExport::class, compact('request'));  
        return $exporter->download('CuisineimgExport_'.date('Y-m-d').'.'.$slug);
    }
}
