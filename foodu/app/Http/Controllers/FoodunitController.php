...
use App\Exports\FoodunitExport;
...
class FoodunitController extends Controller {
	...
	public static function display(  )
	{
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('foodunit.public.view',$data);			
			}			
		} 
		else {
			...
			return view('foodunit.public.index',$data);	
		}
	...
	function store_public( $request)
	{
		...
		public function foodexport(Request $request,$slug) 
           {        	
        $request->all();
        $exporter = app()->makeWith(FoodunitExport::class, compact('request'));  
        return $exporter->download('FoodunitExport_'.date('Y-m-d').'.'.$slug);
    }
