...
use App\Exports\VariationunitExport;
...
class VariationunitController extends Controller {
	...
	public $module = 'variationunit';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Variationunit();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'variationunit',
	...
	public static function display(  )
	{
		...
		$model  = new Variationunit();
		$info = $model::makeInfo('variationunit');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('variationunit.public.view',$data);			
			}			
		} 
		else {
			...
			return view('variationunit.public.index',$data);	
		}
   ...
   public function variationexport(Request $request,$slug) 
           ...
           $request->all();
        $exporter = app()->makeWith(VariationunitExport::class, compact('request'));  
        return $exporter->download('VariationunitExport'.date('Y-m-d').'.'.$slug);
    }
