...
class ModuleController extends Controller {
    ...
    function postCreate( Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if($request->input('creation') == 'manual')
            {
                if($where =="")                
                {
                    return redirect('sximo/module/create')                    
            ...
            if(count($row) >= 1)
            {
                ...
                return redirect('sximo/module/rebuild/'.$id->module_id);        
            } else {
    ...
    function postSaveconfig( Request $request) 
    {
        ...
        if ($validator->passes()) {
            ...
            {
                return response()->json(array('status'=>'success','message'=>'Module Info Has Been updated  Successfull')); 
            } else {

                 return redirect('sximo/module/config/'.$request->input('module_name'))
                ->with('message','Module Info Has Been updated  Successfull')->with('status','success');
            } 
    ...
    public function postSavesetting( Request $request)
    {
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'Module Setting Has Been Save Successfull')); 
             ...
             return redirect('sximo/module/config/'.$row->module_name)
    ...
    function postSavesql( Request $request ,$id )
    {
        ...
        try {   
            ...
            // Do something when query fails. 
            $error ='Error : '.$select .' '.$where.' '.$group ;
            return redirect('sximo/module/sql/'.$request->input('module_name'))
       ...
       \DB::table('tb_module')
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'SQL Has Changed Successful.')); 
             ...
             return redirect('sximo/module/sql/'.$row->module_name)
    ...
    public function postSaveform( Request $request)
    {
        ...
        if($request->ajax() == true)
        {
            ...
            return redirect('sximo/module/form/'.$row->module_name)
    ...
    public function postSavetable( Request $request)
    {
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'Module Table Has Been Save Successfull')); 
             ...
             return redirect('sximo/module/table/'.$row->module_name)
   ...
   function getPermission( $id )
        ...
        */         
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'Permission Has Changed Successful')); 
             ...
             return redirect('sximo/module/permission/'.$row->module_name)
    ...
    public function postFormdesign( Request $request)
    {
      ...
      \DB::table('tb_module')
        ...
        return redirect('sximo/module/formdesign/'.$row->module_name)
    ...
    function postSavesub( Request $request)
    {
        ...
        if ($validator->passes()) {                
            ...
            if($request->ajax() == true)
            {
                ...
                return redirect('sximo/module/sub/'.$row->module_name)
                ->with('message','Master Has beed added Successful.')->with('status','success');   
            } 

        }    else {

            return redirect('sximo/module/sub/'.$request->get('module_name'))
            ->with('message', 'The following errors occurred')->with('status','error')
            ->withErrors($validator)->withInput();
   ...
   function configGrid ( $field , $alias , $type, $sort ) {
    ...
    function postDobuild( Request $request ,$id )
    {
        ...
        $url = 'sximo/module/rebuild/'.$id."?rebuild=y&c={$c}&m={$m}&g={$g}&f={$f}&v={$v}&fg={$fg}&fv={$fv}&ff={$ff}";  
   ...
   function postSavesubform( Request $request)
    {
        ...
        if ($validator->passes()) {                
            ...
            $row = \DB::table('tb_module')->where('module_id', $id)
                                    ->get();
            if(count($row) <= 0){
                return redirect('sximo/module/subform/'.$request->get('module_name'))
                    ->with('message', 'Can not find module')->with('status','error');      
            }
            $row = $row[0];                                    
            $this->data['row'] = $row;    
            ...
            */
            ...
            if($request->ajax() == true)
            {
                ...
                return redirect('sximo/module/subform/'.$row->module_name)
    ...
    function getSource( Request $request ,$id)
    {
        ...
        return view('sximo.module.source',$this->data);
        ...
        if ($validator->passes()) {
            ...
            if(count($res)>=1)
            {
               ...
               // echo $row->module_id ; exit;
                ...
                return redirect('sximo/module/rebuild/'.$row->module_id.'?mode=duplicate');
         
            } else {
                return redirect('sximo/module')->with('message','Failed to Duplicate Module !' )->with('status','error');
