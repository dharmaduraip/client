...
class SximoHelpers {
  ...
  public static function upgrade($zip_path) { 
  ...
  static public function showForm( $module = null )
  {
     ...
     if(count($sql)>=1)
     {
        ...
        if(file_exists(base_path()."/resources/views/".$row->module_name."/public/form.blade.php"))
        {
          return view($row->module_name.'.public.form',$data);
        } else {
  ...
  Static public function sjForm( $id ) {
    $id = $id ;
    ?>
