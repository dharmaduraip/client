...
class Orderdetails extends Sximo  {
	...
	public static function queryWhere(  ){
		...
		return "  WHERE abserve_order_details.id IS NOT NULL AND abserve_order_details.status = '0'";
	...
	function getStatusTextAttribute()
	{
		...
		return \SiteHelpers::getStatusTiming($status);
	}
	...
	*/
	...
	function getApiStatusTextAttribute()
	{
		...
		if($status == '0' || $status == 'pending'){
			...
			$text = 'Rider Arrived customer location';
		}
		return $text;
	}
	...
	function getFoodAvailableCountAttribute()
	{
		...
		$data['unavailableCount'] = $unavailableCount;
	...
	function getCustomerDetailAttribute()
	{
		...
		if($user_id > 0){
			$select = ['phone_number','username as name'];
	...
	{
		return \DB::table('abserve_rating')->select('rating','comments')->where('orderid',$this->id)->first();
	}

	function getBoyRatingInfoAttribute()
	{
		return \DB::table('abserve_rating_boy')->select('rating','comments')->where('orderid',$this->id)->first();
	}
